...
MUTATIONS: list[tuple[str, str, str, str, str]] = [
    ...
    # ---------------------------------------------- N 组:写死的数
    #
    # ## 这两条的锚点**不许包含被守卫读的那个数**(a54 修)
    #
    # 原来 N1 的锚点是 `"一共 19 份。"`。a53 往文档地图里加了
    # `docs/LOG-CONSOLE.md` 的那一刻它就失锚了 —— 表里变成 20 行,那句话跟着
    # 变成「一共 20 份」,而锚点还在找 19。`audit_anchors.py` 因此变红,
    # 而在它变红之前,这条变异**什么都没验**:这份脚本开头写着文档类守卫
    # 特有的失效方式是「在报告里占一行 PASS」,那件事发生在了它自己身上。
    #
    # 重新对准成 20 是没用的:下一份文档加进来会再犯一次,这类锚点的寿命
    # 等于「下一次有人加一份文档」。所以改成锚在**不含数字的那一段**上,
    # 把一个过期的数插到前面 —— 守卫读的是 `re.search` 的第一个命中,
    # 于是「说的数」和「表格行数」照样分叉,而锚点不再随文档增删过期。
    #
    # N2 是同一个病灶,今天还没发作(眼下真的是 9 份),一并改掉。
    (
        "N1",
        "文档地图那句话与表格行数分叉(改成 13,正是 a46-phase5 修前的状态)",
        STATUS,
        "## 七、文档地图\n\n一共 ",
        "## 七、文档地图\n\n一共 13 份(这个数停在 a46-phase5 之前)。实际一共 ",
    ),
    (
        "N2",
        "地图外过程文档的条数写回旧值(改成 29,大批收编之前的状态)",
        STATUS,
        "过程结论(眼下 ",
        "过程结论(眼下 29 份,这个数停在收编之前;真实是 ",
    ),
    (
        "N3",
