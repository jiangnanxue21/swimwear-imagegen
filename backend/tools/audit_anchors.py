...
`tools/mutate_contract_tests.py` **没有 `if __name__ == "__main__"` 保护** ——
它的变异循环写在模块顶层。`import` 它一次,就是当场改写真实工作树里的
`App.tsx` / `TodayPage.tsx` / `flow.py`,再起 18 个子进程跑测试。
...
`CASES` 是 A8/A9 那一轮的老形状,今天只剩 `mutate_contract_tests.py` 用它。
**不认它就等于漏掉 18 条锚点**,而那 18 条指向的全是前端文件 ——
恰恰是最常被重构、最容易失锚的一批。
...
# 这一行照样什么都没验,并且比失锚更隐蔽:
#
# `mutate_contract_tests.py` 的 `run_one()` 用 `getattr(m, name)()` 派发,
# 名字没了就是 `AttributeError`,子进程退出码 1,而它按 `rc != 0` 判定
# "响了"。**测试被删掉,报告上写的是「被抓住」。**
#
