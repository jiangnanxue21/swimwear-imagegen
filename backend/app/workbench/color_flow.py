...
## 合并规则:**商品级与颜色级冲突时,谁说了算**(AC-15 / F-1 的裁定)

AC-15 要求"商品级与颜色级冲突时有明确、唯一的合并规则"。规则分两半,
判据是**这一步的产出对象是 SPU 级的还是行级的**:

    SPU 级(IMAGE_SET / DRAFT)
        一个 SPU 一份。"图片集做完了"是关于**整个 SPU**的断言,
        某个在售颜色没有配图时它就是假的。所以商品级状态**必须**
        已经包含颜色维的阻断 —— 而它确实包含:`MISSING_IMAGE_FOR_VARIANT`
        这类问题码本来就在商品级 `ImageSetFacts.violation_codes` 里,
        由 `flow._no_done_while_blocking()` 这条结构不变量兜住。
        **下压发生在 `flow` 里,不在本层。**

    行级(MATERIAL / ATTRIBUTE / PLAN / COPY)
        向导的作用域是一行 SKU,而一行 SKU 只属于一个颜色。
        "这一行的文案批准了"与"BLU 的文案还没生成"**不矛盾** ——
        它们是两个作用域上的两句真话。把后者下压成前者,运营会在
        一件已经做完的 SKU 上看到红色,而他在这一行上无事可做。
        **不下压;颜色维单独一列,用来回答"先补哪个颜色"。**

## 为什么下压不能放在向导里

放在向导里的话,同一件商品在列表页(读 `flow`)与向导页(读 `flow` + 颜色维)
上是两个状态 —— 而列表页出于 N+1 的考虑不算颜色维,永远补不上这一课。
AC-15 的原话是"三处不得出现互相矛盾的状态",而三处共用的那一份是
`flow.evaluate()` 的结果。**修在共用的那一份上,三处同时对。**
...
#: 颜色子态 -> 商品级步骤状态。**这是一张对照表,不是下压通道。**
#:
#: 它的用处是让两套状态词汇可以互相对照(界面上色、批量筛选按同一档),
#: 而**不是**"把颜色维的结论盖到商品级那一行上" —— 后者见本模块顶部
#: 「合并规则」一节的裁定。
#:
...
# `rollup_step_states()` 曾经在这里。**它被删掉了,这是一次决定,不是遗漏。**
#
# 它的文档写着「商品级那一行的状态要能被颜色维**下压**」,而
# `wizard.worst_color_state()` 的文档写着「颜色维单独一列显示,**不下压**」——
# 两份规格互相矛盾,而两边都没有生产调用点。审阅(F-1)点名的正是这个状态:
# 规则写了,没接;而"一个算好了没人读的模块,和没有它是一样的"。
#
# 裁定见本模块顶部「合并规则」一节:**下压发生在 `flow` 里,不在这一层**。
# 保留一个没人调的翻译函数,只会让下一个人以为下压还没做而去接它 ——
# 接上之后向导的状态就与列表页分叉了,而那正是 AC-15 禁止的第三种状态。
