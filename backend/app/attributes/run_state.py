...
def scope_of(asset: Any) -> str | None:
    ...
    同一处理:那一列是阶段 2 的归属外键,**今天还不存在**。写成属性访问会在
    列落库前当场 AttributeError,写死 `None` 则会在列落库**之后**继续瞎。

    **不读 `variant_hint`。**表上今天只有它,而 §4.8 明写它是「识别建议位」——
    拿它分作用域等于让模型猜出来的值决定**重试范围**,而重试范围决定
    下一次付多少钱。14-9 与 14-10 两次拒绝接线是同一条理由,这里是第三次。
    """
