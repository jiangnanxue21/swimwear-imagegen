...
#:
#: **今天这条截断走不到。** 属性校验层的 `validation.MAX_TEXT_LENGTH` 也是
#: 128,超长的值在 `confirm()` 里就被拒了,根本到不了投影 ——
#: 这一行原来写着"截断而不是拒绝,因为不该让显示副本挡住事实源",
#: 那句话是错的:拦它的从来不是投影,是校验层。
#:
...
def projected_name(value: Any) -> str | None:
    ...
    这个区分是本模块最容易被简化掉的一处:`str(value or "")` 一行就能"работать",
...
def needs_update(current: str | None, wanted: str | None) -> bool:
    ...
    上一版这里写着「不这么做的话每次确认都会让 `color_variants` 进 UPDATE,
    `updated_at` 跟着动」。**那句话是错的**,而且是被变异验出来的:
    SQLAlchemy 的脏检查本来就会把"赋一个和当前值相同的值"折叠掉,不发 UPDATE。
    所以 `current != wanted` 这一半在今天不承重 —— 变异把它拿掉,
    行为完全一样,没有任何守卫会红(batch23 的 B3)。
