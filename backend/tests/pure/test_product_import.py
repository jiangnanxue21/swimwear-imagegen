...
_HEADER = ",".join(
    [
        "spu", "sku", "name", "category", "primary_color", "secondary_colors",
...
GOOD_CSV = f"""{_HEADER}
SPU-001,SKU-001-S,基础三角比基尼,swimwear,black,white|gold,BIKINI_SET,SOLID,HALTER,V_NECK,TIE_BACK,涤纶/氨纶,SIMPLE
SPU-002,SKU-002-M,印花连体泳衣,swimwear,blue,,ONE_PIECE,FLORAL,SPAGHETTI,SCOOP,OPEN_BACK,锦纶,MEDIUM
"""
...
def test_invalid_enum_is_reported_not_silently_replaced():
    csv_text = "spu,sku,name,garment_type\nSPU-1,SKU-1,泳衣,BANANA\n"
...
def test_enum_is_case_insensitive():
    result = parse_csv("spu,sku,name,garment_type\nSPU-1,SKU-1,泳衣,one_piece\n")
    assert result.ok, result.errors
...
def test_bad_row_does_not_block_good_rows():
    csv_text = (
        "spu,sku,name,complexity\n"
        "SPU-1,SKU-1,好行,SIMPLE\n"
        "SPU-2,SKU-2,坏行,VERY_HARD\n"
        "SPU-3,SKU-3,另一好行,MEDIUM\n"
    )
    result = parse_csv(csv_text)
    assert len(result.rows) == 2
...
def test_comma_separated_secondary_colors_also_work():
    row, errors = parse_row(
        {"spu": "S", "sku": "K", "name": "N", "secondary_colors": "red, blue"}, 1
    )
    assert not errors
...
def test_json_rows_share_the_same_validation():
    result = parse_json_rows([
        {"spu": "SPU-1", "sku": "SKU-1", "name": "泳衣", "pattern_type": "stripe"},
        {"spu": "SPU-2", "sku": "SKU-2", "name": "泳衣2", "pattern_type": "NOPE"},
    ])
    assert len(result.rows) == 1
