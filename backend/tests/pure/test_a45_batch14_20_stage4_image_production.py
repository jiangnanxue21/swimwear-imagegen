...
def test_the_plan_panel_is_written_but_not_reachable_yet_and_this_is_the_ledger():
    ...
    **一条过期的理由比没有理由更糟**:照着"写一行 import"去做的人,
    ...
    它测的是更弱也更确定的一件事:有没有人 import 过。

    接线那天这条会红 —— 那时删掉它,把可达性交给 Playwright。
    """
    src = PROJECT_ROOT / "frontend" / "src"
    written = {"GenerationPlanPanel", "generationPlans"}
    importers = {
        path.relative_to(src).as_posix()
        for path in src.rglob("*.ts*")
        if path.name not in {"GenerationPlanPanel.tsx", "generationPlans.ts"}
        and any(name in path.read_text(encoding="utf-8") for name in written)
    }
    assert importers == set(), (
        f"方案面板已经被 {sorted(importers)} 接进去了 —— 这条欠账还清了。"
