...
来源是已删除的过程文档（阶段验收报告、里程碑、评审记录、逐轮 CHANGES）。
那些文档记的是**当时做了什么**；这里只留**现在仍然是什么**。

## 怎么查一条决策

文件分两块,编号体系也是两套,查号时两处都要看:

```
一、二、三 三节      按主题分组的早期结构,小节编号形如 `3.5`
§ 条目               后来逐条追加的决定,标题形如 `## §3.5x`,全仓按 §编号引用它们
```

两套编号在 `3.15`~`3.20` 这一段**重叠**。重叠是历史遗留:改编号会打断全仓大量
`§3.x` 引用,所以留着,由这句话把它说出来 —— 而不是让下一个人自己撞上。

下面的索引由 `backend/tools/gen_decisions_index.py` 从正文生成,覆盖全部 §条目。
**不要手工改索引里的任何一行**,下一次生成会把它抹掉;标签不对就去改那个脚本里的
主题表。`verify_delivery.py` 盯着索引与正文不许分叉。

<!-- DECISIONS_INDEX:BEGIN -->

| § | 标题 | 主题 |
| --- | --- | --- |
| §3.15 | A43:执行归属由令牌决定,不由时钟决定(BLOCK-02) | 数据与身份 |
| §3.16 | A43:属性读取的回落链是临时的(BLOCK-03) | 评分与识别 |
| §3.17 | A43 引入的已知遗留:变体分层挂在一个不稳定的 ID 上 | 数据与身份 |
| §3.18 | A44:变体的身份与名字分开(BLOCK-04 第一件) | 数据与身份 |
| §3.19 | A44:VARIANT 属性的 owner_id 必须带 SPU 命名空间 | 评分与识别 |
| §3.21 | A45:受众是一根独立的轴,不是品类的子分类 | 数据与身份 |
| §3.22 | A45-batch13:身份从字符串约定改成外键(阶段 1 第一批) | 数据与身份 |
| §3.23 | A45-batch13-2:变体身份收成三级,以及回填不是一次数据迁移 | 数据与身份 |
| §3.24 | A45-batch14:识别的判定与落库分家(阶段 3 第一批) | 评分与识别 |
| §3.25 | A45-batch13-3:ORM 不许软化数据库约束;副本编辑必须向上同步权威 | 数据与身份 |
| §3.26 | A45-batch14-2:写在注释里的前提不是前提;规矩要么是不变式,要么是散文 | 文档与门禁 |
| §3.27 | A45-batch14-11:队列成员资格是一张穷举表;拦截作用域必须自己论证,不许沿用去重键 | 数据与身份 |
| §3.28 | A45-batch14-12:守卫要钉性质,不钉做法;派生状态可以先于列落地 | 文档与门禁 |
| §3.29 | A45-batch14-13:不可达的防御代码等于没有;并发的几条线不许各自动迁移链 | 批次执行 |
| §3.30 | A45-batch14-14:守卫也要被审;规矩要挑那条不响的失效 | 文档与门禁 |
| §3.31 | A45-batch14-17:点名做法的守卫会因为进步而变红;两处真相修不成一个补丁 | 文档与门禁 |
| §3.32 | A45-batch14-18:算出来没人读的数,和没算是一回事;点名做法第四次 | 文档与门禁 |
| §3.33 | A45-batch14-19:防的是明天的调用点;守卫点名做法第五次;门禁自己有盲点 | 文档与门禁 |
| §3.34 | A45-batch14-20(阶段 3 · 识别 run 身份):欠账守卫是有还款日的;docstring 是源码的一部分 | 评分与识别 |
| §3.35 | A45-batch14-20(阶段 4):判定写在测不到的地方等于没写;回落逻辑在维度增加后会变成静默的错 | 数据与身份 |
| §3.36 | A45-batch14-20 并线合并:门禁的覆盖面等于有人付过代价的地方;"没写"不许写成"验不到" | 文档与门禁 |
| §3.37 | A45-batch14-21:欠账守卫的还款日要有人替它盯;守卫说出那件事的名字不等于它就是那件事 | 文档与门禁 |
| §3.38 | A45-batch14-22:"读得到、判得了、写不进"是一个可以点名的类;验收要有对象 | 文档与门禁 |
| §3.39 | A45-batch14-23:一条守卫同时钉不变量与现状时,它对现状的那一半是伪装成成绩的欠账 | 文档与门禁 |
| §3.40 | A45-batch14-24:一条只在有人手工复核时才会发现的规矩,等于没有规矩 | 文档与门禁 |
| §3.41 | A45-batch14-26:决定可以由写的人做,但它必须以决定的身份出现 | 数据与身份 |
| §3.42 | A45-batch15-merged:一句说缺口已关的话,比一句过期的话贵得多;过程文档删掉的同时,结论要有去处 | 文档与门禁 |
| §3.43 | A45-batch14-28:「上游算对了、没人接最后一跳」是一种测试看不见的缺陷 | 文档与门禁 |
| §3.44 | A45-batch14-28:切身份之前,先把"静默"拆掉,而不是赌顺序装对了 | 数据与身份 |
| §3.45 | A45-batch14-28 补丁审核:身份迁移必须同时守住作用域与唯一答案 | 数据与身份 |
| §3.46 | A45-batch17-1 补丁审核:增量补丁的前置条件也是合入契约 | 数据与身份 |
| §3.47 | A45-batch17-2 评审修复:落库端的归属校验是一条独立的防线,幂等键顶不了它 | 发布链路 |
| §3.48 | A45-batch18:接线欠账的理由必须准,准不了就别写 | 文档与门禁 |
| §3.49 | A45-batch18 评审修复:门禁扫的是工作树,而交付的是版本库 | 文档与门禁 |
| §3.50 | A45-batch19(阶段 5 批次 5-1):快照列的默认值决定了它是防线还是装饰 | 批次执行 |
| §3.51 | 阶段 5 / 5-1 评审复核：自描述要落到运营能读懂的键，接线前先把爆炸半径分开 | 前端 |
| §3.52 | A45-batch20(阶段 5 批次 5-2B):接线是一跳,而这一跳没有任何既有测试走过 | 批次执行 |
| §3.53 | A45-batch21/22/23(阶段 5 收官):三笔账,以及同一个坑的第七到第九次 | 数据与身份 |
| §3.54 | A45-batch24(三位外部评审的复核):接了一半的三条接缝 | 文档与门禁 |
| §3.55 | A45-batch25(阶段 6 批次 6-1):先立判据,再动最底下那一层 | 批次执行 |
| §3.56 | A45-batch26(6-2 前置):照着自己写的方法跑了一遍,红名单是空的 | 数据与身份 |
| §3.57 | A45-batch27(阶段 6 批次 6-2):七步增维,以及一段来路不明的半成品 | 批次执行 |
| §3.58 | A45-batch28(阶段 6 批次 6-3):聚合工作流 API —— 那笔明说的账还了 | 批次执行 |
| §3.59 | A45-batch29(阶段 6 批次 6-4):七步向导、刷新恢复,以及一个被位置参数吞掉的判定 | 批次执行 |
| §3.60 | A45-batch30(阶段 6 批次 6-5):费用预估与影响提示 —— 两个"不许给 0" | 批次执行 |
| §3.61 | A45-batch32(阶段 6 回归修复):兜底网接错了位置,以及一条从来没有人断言过的不变量 | 文档与门禁 |
| §3.62 | A45-batch33(阶段 6 回归修复续):一个后端收得下、前端填不出的字段 | 前端 |
| §3.63 | F-17 修复:生成方案按作用域保留一个 ACTIVE 与一个可编辑 DRAFT | 数据与身份 |
| §3.64 | F-12/F-4 修复:颜色码由后端解析成操作目标,URL 保存操作上下文 | 数据与身份 |
| §3.65 | 人工测试准入收口：身份先行、异步识别与签名预览 | 评分与识别 |
| §3.66 | 浏览器登录:滑动过期,以及浏览器侧具名审计的丧失 | 前端 |
| §3.67 | 浏览器登录的前端接线:未登录该往哪儿去,由后端说 | 前端 |
| §3.68 | 浏览器登录的可部署性:一个必填到"不填就起不来"的配置,全仓 0 份文档 | 前端 |
| §3.69 | 打包复验的假阴性:grep -q 提前退出 × SIGPIPE × pipefail | 打包与凭据 |
| §3.70 | 文档审核:四类缺陷,一个共同点 —— 两份真相分叉,而没有东西在看 | 文档与门禁 |
| §3.71 | 浏览器登录收尾:Token UI 退役、菜单按角色收敛,以及八条为 Token 写的门禁 | 前端 |
| §3.72 | 界面收口 a47:方案真正控制出图,菜单 13 → 7,款级聚合不编下一步 | 前端 |
| §3.73 | a48 收口:离线看得见死 import,方案接管的语义补齐两个调用方 | 文档与门禁 |
| §3.74 | a49 评审整改:十二条,加上三份从来没有跑过的门禁 | 文档与门禁 |
| §3.75 | 评审整改(REVIEW II.8 / III.2 / III.6 / II.1):四条能在离线门禁里真跑的,加上两条只能记下来的 | 文档与门禁 |
| §3.76 | 视觉评分的限制对象是整份请求,不是单张图片 | 评分与识别 |
| §3.77 | a51:建档的四类失败提前到"下一步",以及 SPU 停用补上一个到不了的状态 | 数据与身份 |
| §3.78 | 界面用语归一:后端枚举不许直接摆在界面上,英文名一律给中文对照 | 前端 |
| §3.79 | A52:界面上的英文标识符、一条走不通的复活路径,以及 a51 走查的九条 | 前端 |
| §3.80 | a53 运行日志控制台:归类、展示、原文,以及一条从来没有人读过的日志 | 日志观测 |
| §3.81 | a54:运行日志控制台的十处修复,以及一条锚在会变的数上的变异 | 日志观测 |
| §3.82 | 属性识别沿用评分链路的整份请求预算,不是改走 Batch | 评分与识别 |
| §3.83 | 逐图 missing 不是整批 missing:正面看不到背部不等于缺背面图 | 评分与识别 |
| §3.84 | 属性识别的“重新识别”是显式新 run，普通重复请求仍然幂等 | 评分与识别 |
| §3.85 | 未校准只禁止自动确认，不得吞掉模型候选值 | 评分与识别 |
| §3.86 | 空属性也必须有编辑行；枚举存英文编码、显示中文 | 评分与识别 |
| §3.87 | a55 运行日志控制台整改 | 日志观测 |
| §3.88 | 行尾策略收敛为单一实现,打包脚本对成包做行尾复验;日志控制台五修 | 打包与凭据 |
| §3.89 | 提示词注册表与内容派生版本落地(BLOCK-30 的 S1 + S3 后端核心) | 评分与识别 |
| §3.90 | a57:AI 测试留档落地,一份指着不存在的表的迁移,以及一条失锚了两轮的变异 | 文档与门禁 |
| §3.91 | a58:提示词管理中心的前端半,男装那笔欠账还清,以及连着两轮由变异抓出的同一种守卫 | 前端 |
| §3.92 | a59:同一种守卫缺陷第四次出现,于是把它交给机器;以及两处我自己写出来的分叉 | 文档与门禁 |
| §3.93 | a60:S4 的乐观锁与回滚理由,以及一次差点顺手抹掉别人欠账的改动 | 文档与门禁 |
| §3.94 | a61:前端第一条会执行的测试通路,以及一句重复了六遍的话 | 前端 |
| §3.95 | a62:BE-311 落地,顺带把三笔"前提已经满足"的欠账一起还了 | 文档与门禁 |
| §3.96 | a63:两列版本的语义一起塌了,而两道防线都没看见 | 数据与身份 |
| §3.97 | a64:把 a63 那个缺陷做成门禁,以及一条锚点连着两轮被同种改动打断 | 文档与门禁 |
| §3.98 | a65:真库那一侧也有余地 —— 迁移链能在内存里跑 | 数据与身份 |
| §3.99 | a66:列型比对当天抓到一条会当场炸的缺陷,以及我自己第二次造的状态分叉 | 数据与身份 |
| §3.100 | a67:第一次真的还了棘轮的债,而后半程不是机械替换 | 文档与门禁 |
| §3.101 | a68:一项任务从 v1.0 起就没进过任何一格,而两张表各自都自洽 | 数据与身份 |
| §3.102 | a68:外部审计四条落地,以及"门禁全绿"到底证明了什么 | 文档与门禁 |
| §3.103 | a69:报错文案是界面的一部分,而它此前不在任何门禁的射程里 | 前端 |
| §3.104 | a70:统计的分母比统计本身难,以及"数据已齐"那句话错在哪 | 日志观测 |
| §3.105 | 按场景优化默认提示词:先修消费链路,再修措辞(2026-08-19) | 评分与识别 |
| §3.106 | 提示词中心:不可保存不等于不可查看(2026-08-19) | 评分与识别 |
| §3.107 | 过程文档与一次性脚本的留存口径 | 文档与门禁 |
| §3.108 | 两条工作线合流:文档重构与代码整改的交叉核对 | 文档与门禁 |
| §3.109 | 两个审核入口都留,判据是审的对象不同 | 数据与身份 |
| §3.110 | 注释口径改成「陈述契约,链接理由」,并给出可判定的四条 | 数据与身份 |
| §3.111 | 取证台账:三列各答一个问题,状态由三列算出 | 数据与身份 |
| §3.112 | 解锁闸:散在六处的取证标志必须汇成一句话,而且默认是拒绝 | 数据与身份 |
| §3.113 | 三条"不许把不知道说成知道、也不许把知道说成不知道" | 数据与身份 |
| §3.114 | 外部复审四条:门禁全绿不代表事实被验过 | 文档与门禁 |
| §3.115 | 复审第二轮:判据要写成白名单,以及"证明"与"计数"不是一回事 | 数据与身份 |
...
## 三、升级须知已独立成 `UPGRADING.md`

**从版本 X 升级需要人工做的动作,全部在 [`UPGRADING.md`](UPGRADING.md)。**
原来的 3.1~3.4 四节(必须做的人工动作 / 原本能跑通现在会被挡下的操作 /
行为变了但不报错的看板口径切换点 / 部署形态变更)整体搬了过去,一个字没删。

搬走的理由不是它们不重要,恰恰相反:要升级的人只想要那一份清单,而它埋在
一份七千行的决策日志的第三节里,前面还压着迁移编号和二十条架构约束。

下面 3.5 起的各节留在原地 —— 它们记的是**决定**,不是升级动作,而且全仓有
大量按 §3.x 的引用指着它们。
...
## §3.106 提示词中心:不可保存不等于不可查看(2026-08-19)

提示词中心原来的出发点是避免假保存:当时只有两份评分系统 Prompt 读取
`prompt_templates`,其余项直接开放保存会得到「页面说成功、运行时一个字都不读」。
但把“消费方尚未接线”当成产品层长期只读的理由不合理，尤其文案、属性识别和出图
本来就是稳定文本/模板。界面还把这条边界实现成了**只列元数据、不展示正文**,
又只靠提示词名称上的链接暗示可编辑入口。结果运营看到的是一张代码盘点表,不是提示词
管理页:不能判断默认正文是什么,也看不出哪些项可以进入编辑页。

本轮保留“可编辑 = 消费链路读版本库”的定义和保存端点闸门,不制造假保存；同时要求
注册表每一项都能解析出 `content_preview`。静态字符串展示原文,策略映射序列化为 JSON,
运行时构造器提供一个无参代表性预览函数,并由 `preview_note` 说清示例输入与生产输入的
差别。列表页给可编辑项明确的「查看并编辑」按钮,其余项给「查看正文」并展开只读文本。
这样“能不能保存”和“能不能看见”成为两条独立、都如实表达的能力。

同时完成三条消费接线，而不是只翻 `editable` 布尔值：文案生成器每次调用前读取系统
模板；属性识别在**计算幂等键之前**读取带 `{field_lines}` 的模板，保证改 Prompt 会换
识别身份；出图在应用方案时读取四槽位拼装模板，最终正文仍完整落库并按内容派生版本。
三者都复用既有版本历史、乐观锁、恢复默认和 TEMPLATE 槽位体检。

随后补齐最后三条消费链路，不能以“风险较高”为理由永久做成只读：评分用户段重构成
保留 `{depth_instruction}` / `{dimension_lines}` / `{codes}` / `{image_lines}` /
`{check_lines}` / `{metadata_text}` 六个代码槽位的模板；默认模板展开结果与重构前一致，
运行时只替换登记槽位，不用会误伤商品 JSON 花括号的 `str.format`。评分深度指令改为
FULL / QUICK 两键的 JSON 映射。两者都由 `evaluation_service` 在每轮开始时读取活动版本，
再经 `rule_set` 注入无数据库依赖的视觉评分器。

修复提示词补丁表同样以 JSON 版本化，但“可编辑”只开放值，不放弃代码拥有的结构：
必需问题代码从 `REPAIR_TABLE` 惰性派生，解析器校验布尔字段、字符串数组、参数对象与未知
字段；有效映射由服务层按轮快照，显式传入纯决策函数和 `build_repair_plan`。三类运行时
配置都在付费评分请求前读取/解析；生产环境读取或结构校验失败时转人工，不在不确定口径
下继续花钱或自动放行。至此注册表 8 处全部能查看、编辑、保留历史并切回旧版本；
`editable` 仍严格表示“真实消费方读取版本库”，不是 UI 权限装饰。


## §3.107 过程文档与一次性脚本的留存口径

### 一、变异与复核脚本随批次交付后即删

`backend/tools/` 下曾同时躺着 39 份 `mutate_*.py` 与 4 份复核脚本。它们的产出已经
固化在源码里:守卫改成了什么形状、哪一条断言是被变异逼出来的,都写在对应的测试与
注释里。脚本本身对后续开发者是噪音,而且每一份都带着一组会过期的锚点。

从此:变异与复核脚本随对应批次交付后即删,不留档,不另建 archive 目录。
需要考古时走 git 历史。

三条随之改口径:

```
audit_anchors.py                     零脚本不再判失败,但必须在输出里明说
verify_delivery.py                   新增「一次性脚本没有回潮」,盯 backend/tools/ 下的 mutate_* 前缀
test_a45_batch14_3_fixes.py          「对象为空不是通过」那条断言随之翻面,理由写在用例里
```

被否定的是**把跑完的一次性脚本留在树里**,不是变异验证本身。变异验证抓过失锚、
抓过"守卫在说假话",那些结论留在各自的测试里。

### 二、REVIEW-* / MERGE-* 过程文档删除,结论已在别处

总纲一直写着「过程文档不留档 —— 结论进 `docs/DECISIONS.md`」,而 `docs/` 下存着
11 份按批次留下的评审/合并文档。总纲与现实相反了很久。

下面 9 份删除。每一份的结论性内容(仍在生效的约束、AC 证据指针)此前已经沉进
本文件的对应条目或 `docs/subsystems/`;这里逐份记下原文件名,便于按 git 历史检索:

| 原文件 | 结论今天在哪 |
| --- | --- |
| `REVIEW-A44-A45-MERGED.md` | 合并台账;逐条结论已进本文件 §3.18~§3.21 一族 |
| `REVIEW-A44-BATCH7.md` | 第五节的三类处置理由 -> `tests/pure/test_a44_batch7_fixes.py` 顶部 |
| `REVIEW-A45-BATCH12-2.md` | 该轮结论已被 batch12-3 更正,以后者为准 |
| `REVIEW-A45-BATCH12-3.md` | 四条回头修 -> `tests/pure/test_a45_batch12_3_fixes.py` |
| `REVIEW-A45-BATCH14-3.md` | 锚点审计的来路 -> `backend/tools/audit_anchors.py` 顶部 |
| `REVIEW-STAGE5-5-1-CONCLUSION.md` | 切法与认领清单 -> §3.51、§3.53 |
| `REVIEW-STAGE6-CONCLUSION.md` | 分批、判据与四件欠账 -> §3.54~§3.61 |
| `MERGE-A45-BATCH14-20-STAGE4-IMAGE-PRODUCTION.md` | 阶段 4 五项交付 -> §3.35、§3.36 |
| `MERGE-A45-BATCH18-STAGE4-PLAN-HOST.md` | 方案面板宿主页 -> §3.48 |

两份**不删**:`REVIEW.md` 是施工方案(`verify_delivery.py` 引用它的节号),
`REVIEW-EXTERNAL-2026-08-12.md` 是外部审计原件,移入
`docs/notes/2026-08-12-external-code-review.md` —— 原件和过程结论不是一回事。

### 三、`HANDOVER.md` 只留最近一轮

改成:**只保留最近一轮交接(<= 300 行)**;写入新一轮时,把上一轮迁进
`docs/notes/YYYY-MM-DD-handover-<轮次>.md`。

理由和上面两条同源。交接文档回答的是「刚才那一轮改了什么、验了什么、**哪些没验**」,
它的读者是接手下一轮的那个人,而那个人只需要最近一轮。更早的十几轮不是没有价值,
是价值形态变了 —— 它们是档案,而档案不该压在一份每轮都要读的文件顶上。

### 四、`docs/STATUS.md` 正文前置

实测正文原来在第 2696 行,前面是 46 轮逐次追加的评审快照与交接记录。这份文件的
自我定位是「唯一负责回答现在到底能不能用的地方」,而结构与定位正好相反。

快照按轮次切进 `docs/notes/`,文件末尾留一张「历史快照索引」表。
`verify_delivery.py` 读两个 STAGE 标记走的是 grep,对行号没有假设,重排安全。


## §3.108 两条工作线合流:文档重构与代码整改的交叉核对

《PRD:仓库卫生(P0)与文档重构(P1)》与《前后端代码问题清单》是两份独立立项、
并行推进的文档,各自的交付在这一批合流。合流时逐条核过下面四处,结论记在这里 ——
代码整改那一批交付时**没有拿到那份 PRD**,自己在交接里点了名说这是推断不是核对,
所以核对结果要有落点。

### 一、`backend/tools/` 的清理与新增不冲突

H-3 清退的是**跑完的一次性脚本**:39 份 `mutate_*.py` 加 4 份复核脚本。
新增的 `tools/provider_smoke.py` 是常驻工具 —— 它有稳定入口(`run` / `record`)、
有 make 目标、有守卫,和 `verify_delivery.py` / `audit_anchors.py` 同类。

H-3 的防回潮检查(`verify_delivery.py` 的「一次性脚本没有回潮」)只盯 `mutate_*`
前缀与 `recheck/` 目录,所以它不会误伤。**判据写在这里,免得下一个人看到
`tools/` 又多了一份文件时,拿不准该按哪一条读。**

### 二、`audit-anchors` 那一条红随清理一起消失

代码整改那一批交接里记着「交付时仍然红着:`audit-anchors` 640/643 ——
`mutate_a58.py` 锚点不唯一 ×1、过期 ×1,`mutate_batch14_4.py` 过期 ×1,与本清单无关」。

那三条锚点所在的脚本已随 H-3 删除,这条红因此不再存在。**不是被修好的,是对象没了**
—— 记下来是因为「红的消失了」和「问题解决了」不是一回事,而下一个人只看得到绿。

### 三、整改批的快照进 `docs/notes/`,不追加进 `docs/STATUS.md` 正文

那一批原本把 160 行整改记录追加在 `STATUS.md` 末尾。D-3 之后 STATUS 的结构是
「正文置顶 + 历史快照按轮次归档」,所以这一份按同一口径落在
`docs/notes/2026-08-21-status-code-issues-remediation.md`,索引在 STATUS 末尾。

其中**不随时间过期**的两部分留在正文:三类外部集成的真端点验证状态进「已知限制」,
两代审核页的性质判断进「需要你决策」。判据是 D-6 第一条 —— 正文回答「现在是什么」,
轮次记录回答「那天做了什么」。

### 四、`docs/REVIEW-CODE-ISSUES-2026-08-21.md` 留档,并写明退役条件

D-5 删掉的是 `REVIEW-A*` / `MERGE-A*` 那批**过程结论**:事情做完了,结论已经沉进
别处,原件只剩考古价值。这一份不同 —— 它是**还没做完的工作清单**,B-06/B-07/F-04
明写着「本轮不执行,仅记录方向」,X-01 等着一台真机。它和 `REVIEW.md` 同类:
施工方案留着,是因为还有人要照着它做。

退役条件写进文档地图那一行:**全部条目关闭后删除**,结论按老规矩沉进本文件。


## §3.109 两个审核入口都留,判据是审的对象不同

《前后端代码问题清单》F-02 把 `/reviews` 与 `/workbench-review` 判成「一个功能的
两代实现」,理由是两者都 `import '../api/reviews'`。**这个前提不成立**,而结论
跟着它错了一半。

### 一、它们审的不是同一个对象

```
/reviews           一张候选图行不行            对象是 Review(candidate 级)
/workbench-review  这一版图片集 / 文案能发吗   对象是一个商品的下一步动作
```

`WorkbenchReviewPage.tsx` 顶部自己写着这件事,而且写着「合并数据模型是错的」
与「禁止重写任一审核页」。

### 二、共用那个 api 模块是因为**计数**,不是因为重实现

审核中心顶部三条计数里,「候选图审核」那一条取的是 `GET /reviews?status=PENDING`
的 `total`,点进去 `navigate('/reviews')`。**这是调用关系,不是竞争关系。**

只看 import 图会把调用读成重复。判据要落在「审的对象是不是同一个」上。

### 三、没有可抽的共享组件

F-02 的第三步建议「把共用的评审动作抽成共享组件」。查过了:`/reviews` 走
`reviewsApi.approve / reject / regenerate`(candidate 级),审核中心走
`onPick('APPROVE_IMAGE_SET')` 那一族(商品动作级)—— 两套 mutation 没有交集,
抽出来的会是一个只有一个调用方的壳。

### 四、于是这一条的动作只剩两件

```
docs/user/guide.md                            写明「什么时候用哪个」,判据写成审的对象不同
tests/pure/test_review_entry_wiring.py        钉住路由还在、跳转还在、指南还答得出那个问题
```

**没有给 `/reviews` 加废弃标注,也没有删路由。** 加了就是把一个仍在被调用的入口
标成过期,而下一个人会照着标注去删它 —— 那正是这条评审意见差一点造成的结果。

守卫里那条「指南必须写出判据是审的对象不同」不是文档洁癖:两页并存而没人说清
分工,下一轮走查会再次把它读成两代实现。**第一次读错已经发生过了,这条守卫钉的
就是那次读错。**


## §3.110 注释口径改成「陈述契约,链接理由」,并给出可判定的四条

参照 [deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) 的仓库约定
(`AGENTS.md` 的 *Type safety and documentation* 与 *Conventions* 两节)。那份约定
一句话:注释与文档陈述**完整的契约与事实,不是推理过程的誊本** —— 不叙述控制流、
不保存评审史、不复述代码;保留行为、失败、时序、归属与安全使用这几类,理由用链接。

### 一、改的是写什么,不是写多少

本仓的注释密度偏高是刻意的,那一条不动。变的是内容取舍:成段的「它从前是什么样」
从注释里搬进 `docs/notes/`,原位留一句现在时结论。D-1 已经对 `CLAUDE.md` 与
`client.ts` 做过这件事,这一条把它变成全仓约定。

硬规则 5 因此改写:从「注释写为什么,不写是什么」改成「陈述契约与事实,理由用链接
指过去」。前一句在实践里被读成了「把推导过程写下来」,而那正是要改的东西。

### 二、可判定的落到门禁

```
不保存评审史/订正史      tests/pure/test_comment_history_ratchet.py   逐文件棘轮,只许降
空 except 要命名它吞什么  tests/pure/test_silent_except_ratchet.py     基线 0
try 只包一条语句          同上                                          基线 0
文件末尾恰好一个换行      tools/normalize_eol.py                        与行尾策略同一个家
不用比喻                  —— 没有门禁                                   判不出来
```

「不用比喻」刻意不加门禁:它没有可判定的形状,配一条正则只会同时误伤正常表达和
放过真正的比喻。留在 `docs/STYLE.md` 第八条,靠人工评审。

### 三、评审史那一条为什么是棘轮而不是 0

立规矩时全仓有 56 个文件、73 处。一次性重写全部等于在一个批次里动几十处注释,
其中大多数没有人当场复核 —— 那比留着它们贵。所以:立规矩 + 挡住新增 + 逐批还账,
形状与 `ErrorNotice` / `exhaustive-deps` / `aria-label` 那几条棘轮相同。

本批先还了 6 个文件 10 处(`colour_projection.py`、`action_gate.py`、
`generation_plan.py`、`spu.py`、`useIdentity.ts`、`AppLayout.tsx`),基线从 86 起
落到 73。

### 四、这条守卫的射程写在它自己的 docstring 里

它判字样,不判语气,一段没用那些词的叙事段它看不见。写明射程是为了避免下一个人
把它的绿读成「注释史已经清干净了」。

检测器自己那份文件被排除在扫描之外:标记表逐字写着它要抓的东西,举例说明
「不许写什么」时,例子本身会成为违规。

## §3.111 取证台账:三列各答一个问题,状态由三列算出

**来源:** 《PRD:SHEIN 真实渠道适配与自动发布》评审稿 v1.3 的 P-19 / P-20 / P-21 / P-14。
**落点:** `backend/app/channels/shein/sources.py`、`docs/vendor/shein-openapi/SOURCES.md`。

### 一、两个日期不是同一件事

评审 v1.2 在来源矩阵那一列上出过一次错:它把 S-20 的抓取说明挪到了 S-07 头上。
错法本身不重要,重要的是**那一列同时装着两种事实**:

```
页面标注更新时间   官方那边这一页什么时候动过     页面自己声明
本轮抓取时间       我方谁在哪天读过它             取证的人写
内容哈希           那次读到的正文是什么           存档正文的 SHA-256
```

只有页面时间答不出「这条上次何时被我方核对过」;只有抓取时间答不出「官方那边
动没动」。而 S-01～S-31 重新逐页检查之后,复核结果**需要一个落点** —— 混在一列
的矩阵里没有那个格子。

### 二、状态不许手填

`VERIFIED` / `UNVERIFIED` 由三列算出:三列齐全才是已核验。手填的形状是「行说
自己已核验,而哈希那格是空的」—— 一个自称已验证却拿不出证据的条目,比一个明说
没验的条目更贵,因为读它的人会停止追问。

另两档由行自己声明,因为它们不是"缺证据",是"证据说了别的事":`STALE` 必须给出
后继页面 ID(S-07 的 3001350 -> 3001926),`ALIAS` 必须指向 canonical。

### 三、引用得起来的来源必须查得出是哪一页

`capabilities.py` 每条记录带 `sources`,而在此之前那些编号**没有定义处** ——
`S-07` 是合法格式,但没有任何地方回答它是哪一页。`cited_but_unknown()` 恒为空
由守卫钉住:一个引用得到、却查不出是哪一页的来源编号,和没有来源是一回事。

同理,没有标题的行(本仓从未引用过的 S-01 / S-14 / S-15)不许被任何端点引用 ——
读都没读过的来源不能支撑一条端点。

### 四、本轮自查抓到自己两处同形状的错

**这一节是交付前的自查结果,不是事后补的。** 两处错的形状与评审稿 §零 记的三次
完全一样:**把一个充分看起来像证据的信号,当成一个更大结论的证明**。

```
第一列叫「页面标题」   那些字符串是从引用它的端点反推的本仓称呼,不是官方页面上
                       那行字。下一个人拿它去核对页面会核对不上 —— 改叫「本仓称呼」
第二列不分来路         S-07 的 2026-07-10 与 S-20 的 2026-08-21 **都来自评审稿转述**,
                       不是本仓打开过那两页;而台账把它们记成了我方的取证记录
```

第二处的代价具体:`VERIFIED` 会在没有任何人打开过页面的情况下被算出来 ——
而这张表存在的全部理由就是回答「谁在哪天真的读过它」。所以增加第四列 `provenance`
(`OBSERVED` / `RELAYED` / `NO_DATA`),**转述的行永远到不了 `VERIFIED`**。

这一列直接照搬评审稿 §零 的约定。那条约定是评审方为自己立的,而它同样适用于
消费评审结论的这一侧:一条转述被存进台账之后,来路就没有任何地方记得了。

### 五、第三列不能靠手填

`archive_mismatches()` 要求「声称有哈希」的行必须真的有存档文件、且哈希对得上。
当前全部行的哈希都是 None,所以它恒为空 —— 它守的是**第一次有人填哈希那一刻**。
这与 P-14 的存档纪律是同一件事的两半:入仓前先脱敏与密钥扫描,入仓后哈希与正文
不许分叉。

## §3.112 解锁闸:散在六处的取证标志必须汇成一句话,而且默认是拒绝

**来源:** 评审 v1.3 的 P-04。**落点:** `backend/app/channels/shein/readiness.py`。

### 一、能被单独读到,不等于有人回答过

取证标志此前散在六个模块里(签名示例向量、响应形状、回调验签、回调媒体类型、
特殊字符清单、spec TODO)。每一个都能被单独读到,而**没有任何一处回答"那到底
能不能发"**。散着问的结果是每个调用点各答一次,而其中一处答错就是一次真实的创建。

闸把六处的阻断项**原样**串起来,不重写措辞。重写的那一份改了不会跟着变 ——
`shein.describe()` 的 `ready` 从前是一个字面 `False`、`blocked_on` 是一句手写的话,
两者都对,但它们对的方式是"有人当时记得写对"(硬规则 4 的反面教材)。

### 二、三档而不是布尔

```
BLOCKED       连 fixture 都不该跑 —— 缺的是结构性的东西
FIXTURE_ONLY  只允许对 fixture / mock 跑,不发真实请求
REAL          允许真实写操作
```

P-04 说 SH-3 限定 fixture/mock,`FIXTURE_ONLY` 就是那句话的机读形式。用布尔的话
"能不能跑测试"和"能不能发请求"共用一个答案,而 SH-3 的整个价值在于两者分开。

### 三、读操作与写操作的闸不一样

只读端点没有写副作用,解锁条件只有签名与凭据;写操作还要加上响应形状、店铺身份、
绑定证据。合成一个闸的代价是取证期间连「查一下类目树」都做不了 —— 而那正是取证
本身需要做的事。

### 四、闸不读注册表

`channels/registry.py` import 本包,反过来 import 会成环。"有没有 transport"由
注册表那一侧回答,闸只答"就算有 transport,按取证水平该不该让它发"。两个问题分开,
注册表那边的答案变了也不需要改闸。

## §3.113 三条"不许把不知道说成知道、也不许把知道说成不知道"

**来源:** 评审 v1.3 的 P-01 / P-03 / P-05 / P-06。

### 一、P-06:知道的时候不要说不知道

超时之后不许自动重发 CREATE,理由是"平台可能已经收到"。那个"可能"对绝大多数
超时是对的,但对一类失败是错的:DNS 失败、连接被拒、TLS 握手失败 —— 第一个字节
都没离开本机。把两类压成同一个 UNKNOWN 的代价是运营侧的:每一次 DNS 抖动都变成
一张"请人工去平台确认"的工单,而结论必然是"平台上没有"。工单堆起来之后人的处理
方式必然是「全选、重试」—— 那时这条规则就从"防重复"退化成了"拖慢一切,然后被绕过"。

做成布尔同样不行:`sent=False` 会被读成"没发出去",而它在多数客户端里的真实含义
是"不知道"。所以是三档,而**默认与一切认不出的输入都落到最保守那一档**。

### 二、P-01 / P-05:不知道的时候不要说知道

`derive()` 在取值来源未取证时抛错,不返回占位值:占位值会一路流进绑定表和限流桶,
而它们**存得下**任何字符串;等真值换上去的那天,库里已经有一批以占位值为作用域的行。

S-26 的重复信号只说明"平台上有个同货号的东西",它答不出是不是我们的、平台 ID 是
什么、现在还在不在。转成 `BOUND` 会让库里多一条没有外部 ID 的"已上架",而 4.1 节 H
的清理清单正是靠外部 ID 列出来的 —— 那件商品于是从清单里消失。

### 三、P-03:做不到的时候不要做一个假的

双人复核在现有身份模型上做不出来:浏览器登录只认 `admin` / `operator` 两个用户名,
各配一把角色口令;`ADMIN_TOKEN` 命中时身份名固定是 `admin`。二选一选的是**先不做,
并把不可做写成会变红的事实**。

理由是另一个选项的代价被低估了:在现有模型上"做出来"只能是给两次签名各记一个名字,
而那两个名字来自同一把口令。**一个假的双人复核比没有双人复核更危险** —— 它的存在
会让别的控制被放松(「反正有双人复核」),而它挡不住任何东西。

规则本身照样落地并被验过,因为规则与身份模型无关。`supported_today()` 不是手填的
布尔:加进来一个不是角色名的用户名,它自己就翻。


## §3.114 外部复审四条:门禁全绿不代表事实被验过

**来源:** 交付后的外部静态复审(1 个 P0、3 个 P1)。四条都能复现,都已修。
**为什么单独记一条:** 四条的共同点是**它们全都被 `BLOCKED` 状态遮着** ——
自动化门禁一条不红,而取证补齐、闸抬起来的那一刻才会暴露。

### 一、实测动作的答案没有落点(P0)

`shop_identity.derive()` 无条件产 `appid:value`。`GRANULARITY` 那条实测动作能记
「跑过了 / 没跑过」,却记不下**答案是什么** —— 于是无论实测结论如何,作用域都一样。

假如身份其实是店铺级,同一店铺换一个应用就得到第二个作用域,历史绑定查不到,
**重复创建**。这与 P-21 那条「复核结果无处落」是同一个形状:一个问得出、
却存不下答案的问题,等于没问。台账那边加的是三列,这里加的是 `IDENTITY_GRANULARITY`,
并且它是 `UNSOURCED` 时 `derive()` 拒绝派生 —— 跑完实测却不落结论,和没跑一样。

### 二、写好了没人读的检查(P1)

`archive_mismatches()` 存在、被守卫验过,而**没有任何判定路径调它**。于是
「填一个哈希、不存正文」的行照样 `VERIFIED`、照样不产生阻断。

修法是把它接进 `blocking_sources_for()` 的 `VERIFIED` 分支(单点,两条路径都吃到)
加 `structural_blockers()`(整本台账的完整性属于结构性问题:台账不可信的时候,
连 fixture 都不该跑)。**这是 §3.32 那条「算出来没人读的数,和没算是一回事」
的又一个实例,而这次是在一个刚写下的模块里发生的。**

### 三、一把键背了四个事实(P1)

第一版 `binding_key(shop_scope, supplier_sku)` 用**货号**构造 **SPU 级**绑定:
一件双 SKU 的商品会得到两条绑定,而平台上只有一个 SPU。`placement_key` 又把
上下架与价格压成同一个 SKU×site,而冷却是 SKC×site、价格是 SKU×site×currency。

拆成四把带前缀的键。**库存刻意没有键** —— 它的幂等由官方逐行 `idempotencyKey`
承担,自己派生一把等于造第二套幂等语义,而两套只会有一套被平台认。

### 四、`any` 与 `>= 2`(P1)

`supported_today()` 用 `any(...)`:加进来**一个**具名账号就返回 True,而它仍然
凑不出两个人。一个人不构成双人复核,判据必须是**数得够**。

同时补上的是一道更隐蔽的缝:即使有具名登录用户,浏览器身份的来源仍是
`ROLE_PASSWORD`,不在可区分集合里 —— 于是模型「自称支持、而 `review()` 拒绝」。
新增 `PER_USER_PASSWORD` 这一档把两者对齐。


## §3.115 复审第二轮:判据要写成白名单,以及"证明"与"计数"不是一回事

**来源:** 外部静态复审第二轮(3 个 P1、1 个 P2)。四条都能复现,都已修。

### 一、黑名单式判据的失效方式只有一种

`probes_outstanding()` 第一版写的是 `status == NOT_RUN` —— 只有明说没跑的才算
没跑。把三条状态都写成 `TYPO` 之后 `outstanding` 与 `failed` 双双为空,阻断项清空,
身份**派生成功**。一个拼错的字符串把 fail-closed 整个绕过去。

**没被列进黑名单的都算好的,而「所有拼错」是一个无限集合。** 所以判据反过来写:
不是 `PASSED` 的一律算没通过;状态不在枚举里的另报一条(「还没跑」是进度,
「状态是个错字」是数据损坏,处置不同)。`IDENTITY_GRANULARITY` 同理 ——
它不在枚举里时闸说"可以"而 `derive()` 抛错,**而闸与实际能力矛盾比两边都拒绝
更难查**,所以补了一条穷举用例钉住「闸放行 ⟺ 派生得出来」。

### 二、平台幂等键与本地事实身份是两件事

第一版以「库存幂等由官方逐行 `idempotencyKey` 承担」为由不给库存键。那句话本身
没错,**但它回答的是另一个问题**:

```
幂等键   这次写操作是不是同一次意图   来自已持久化的 command id
事实键   这一行库存说的是哪一格事实   店铺 × SKU × 仓位 × 库存类型
```

没有后者,两个仓位的同一个 SKU 在本地是同一行,后写的覆盖先写的,而覆盖不报错。
四个维度全部必填、没有默认值 —— 一个默认仓位会让「没填」和「默认仓」在库里长得
一模一样。维度本身是**转述**(来自复审,S-19 那一页从未被核对过),
所以 `STOCK_FACT_DIMENSIONS_SOURCED` 为 False 并进阻断清单。

### 三、一份名单不是一份凭据清单

`supported_today()` 上一版数的是非角色名的用户名条数。`ann` 与 `bob` 完全可以
共用一把 `OPERATOR_PASSWORD` —— 那时"两次签名"是同一个人输了两遍同一个口令,
而系统会说它已双人复核。

能证明"这两次签名来自两个人"的只有认证主体本身。新增
`PER_PERSON_CREDENTIALS_ISSUED`,它的依据不是一句话而是 `api/auth.py` 现在**怎么
查口令**:`_configured_password` 把用户名映射到角色口令常量,守卫读那个函数体。
有人改成按人查凭据的那天守卫会红 —— 那是提醒翻标志,不是提醒删守卫。

**两个条件的分工:凭据独立回答「一个名字背后是不是一个人」,数量回答「这样的人
够不够两个」。少了前者,后者数的是字符串。**

### 四、接线台账的判据自己也犯了同一个错

复审第三轮点名「门禁口径」。查下来是这条守卫自己:`_has_caller()` 只看
`ImportFrom`,于是往任意模块塞一行 `from app.workflows import shein_image_prep`
(不调用任何东西)就能让那个判定层被判成"已接线",守卫转而要求**把它从台账里
删掉** —— 一个没人用的模块从台账上消失,而消失的方式是一行无用的 import。

**守卫自称验的是「接线了没有」,实际验的是「被 import 了没有」。** 口径比判据宽,
正是 §3.30 那条「守卫要钉性质,不钉做法」。判据改成"真的用到"(属性访问或调用),
并补一条钉住判据本身的用例:只 import 不使用**不算**接线。

两个方向都做了变异验证:塞一行裸 import -> 台账保持不变(绿);真的调用它 ->
守卫当场红,要求删掉台账那一行。

### 五、接线台账是双向的,而这一轮它自己证明了这一点

`site_plan.py` 因为第二条被 `readiness` 读了,`test_every_decision_module_is_either
_wired_or_on_the_ledger` 当场红并点名「台账里却已经接线的」。台账因此删掉那一行 ——
这正是 §3.39 那条「同时钉不变量与现状」的正确形状:现状变了,守卫要求有人来确认。
