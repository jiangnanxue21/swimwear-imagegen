...
## §3.82 属性识别沿用评分链路的整份请求预算,不是改走 Batch

真实千问兼容端点在属性识别请求上返回了与 §3.76 相同的 6 MiB 硬限制。
识别本来只守 `EXTRACTOR_MODEL_MAX_IMAGE_MB=8`:它限制原始文件,而一张图片转成
Base64 后会膨胀约三分之一,再叠加识别提示词与响应 Schema,所以「单图合法」并不
推出「最终 JSON 合法」。识别仍走实时 `responses` / `chat/completions`,一次只放
一个 data URI;这件事与 Batch 无关。

因此新增第二个含义独立的上限:

    EXTRACTOR_MODEL_MAX_IMAGE_MB      原始输入安全上限;超限时解码前拒绝
    EXTRACTOR_MODEL_MAX_REQUEST_MB    最终识别 JSON 上限;默认 5MB

实现沿用评分链路已经跑过真实端点的口径:先构造原请求,用传输层实际发送的
`canonical_json_bytes()` 计数;放得下时原图一个字节不改。超限时用空 data URI
占位请求扣掉提示词、Schema 与其他 JSON 的真实开销,只压缩发送副本,随后按同一
序列化器复核。每轮都从原始 `PreparedImage` 重编码,不反复压上一轮 JPEG。
公网 URL 不消耗内联图片预算;若纯提示词与 Schema 已经超限,则在 HTTP 前失败。

这次没有把预算检查下沉到通用传输层。传输层不知道不同端点的硬限制,也不知道
调用方能否、应否压缩图片;在那一层统一拒绝会把可修复输入变成一条没有修复动作的
错误。共用的仍是实际序列化器与图片压缩原语,预算值和编排分别归评分器、识别器。

## §3.83 逐图 missing 不是整批 missing:正面看不到背部不等于缺背面图

识别一次一张图,而每张图问同一组目标字段。正面图对 `back_style` 返回
`NOT_VISUALLY_DETERMINABLE`、背面图给出有效值是正常结果:前一条要留作逐图证据,
后一条参与合并。原界面却遍历整批 evidence,只要任意一行带 `missing_reason` 就把
字段列进「补对应角度」提示,完全不看同批次另一张图是否已经给值。于是素材明明有
正背面,界面仍要求补背面图。

修复没有删除 missing 证据,也没有改变合并。后端新增纯判定
`attributes.missing_summary.unresolved_missing()`:同一个字段只要本次 run 中存在
任一非空 `normalized_value`,该字段就不再进入商品级缺失提示;逐图 evidence 原样
保留。RUNNING 中后续图片仍可能解决前一张的 missing,所以接口只在终态返回
`unresolved_missing`。前端只按这个字段分组展示,不再从逐图证据推测状态。

## §3.84 属性识别的“重新识别”是显式新 run，普通重复请求仍然幂等

属性页按钮原来调用普通幂等入口。相同素材、字段、模型与提示词已有一条
COMPLETED run 时，接口立即返回旧行；前端没有复用标识，又把旧计数显示成
“识别完成:3 张图,成功 3 条”。用户看到的是一次刚完成的模型调用，实际一次
HTTP 都没有发出。更严重的是，按钮明写“启动 / 重新识别”，却没有任何入口能
表达“我明确要再跑一次”。

修复没有删除防重复付费：`force_rerun` 默认 False，普通调用仍复用 COMPLETED；
属性页的明确点击传 True。它只允许替换 COMPLETED，撞上 QUEUED / RUNNING 仍返回
原 run，双击不会产生两个并发付费任务。重跑时把 base `idempotency_key` 的占用从
旧完成行转给新 QUEUED 行；旧行的输入指纹、作用域、模型和提示词版本仍完整保留。
不能给新行造随机键，否则之后的普通请求仍会命中旧 base key，重跑出的新结果反而
永远不会被复用。

POST 响应新增不落库的 `request_disposition`，区分 NEW_RUN / RETURN_EXISTING /
REUSE_RESULT。前端不再把后两种说成“刚识别完成”。

## §3.85 未校准只禁止自动确认，不得吞掉模型候选值

真实识别任务三张图都成功，背面图明确返回 `back_style=TIE_BACK`，最后属性表却
仍为空。日志中的直接原因是 `attr.merge_no_value`：服务层把未校准的
`system_confidence=None` 用 `or 0.0` 改成零权重，合并层据此判成
`INSUFFICIENT`。这与既有决策表“未校准 → CANDIDATE”互相矛盾：决策表根本
收不到候选值。

现在区分三个语义：同层证据全部未校准时，用等权投票选择候选值，但
`system_confidence` 仍为 None，所以最终只能落 CANDIDATE，绝不自动确认；
未校准与已校准证据混合时，只让已校准证据计权，避免临时赋 1.0 反压真实校准值；
已经算出且明确为 0.0 的证据仍判 `INSUFFICIENT`。模型自报置信度始终只展示，
不参与采信和自动确认。

同一份日志还证明逐图识别可能持续 104 秒。一次性 toast 在任务完成前就会消失，
于是界面看起来像“点了没响应”。属性页现在在 QUEUED / RUNNING 期间持续展示
“已处理 N/总数”，并沿用既有的两秒轮询；是否仍在途继续只读后端 `can_cancel`，
前端不另造终态判断。

## §3.86 空属性也必须有编辑行；枚举存英文编码、显示中文

属性阻断提示要求运营“人工直接填写 material”，但属性表没有 material 行。
原因是注册表的 `fields_for()` 已明确表示“属性页按它出列”，接口却只遍历事实表
中已经存在的值：没有值的字段恰好没有行，也就没有输入框。这是一个自相矛盾的
死路，不是按钮禁用问题。

`GET /attributes` 现在按商品受众返回注册表中的全部适用字段；尚无事实的字段返回
`is_placeholder=true` 的“待填写”视图行，人工填写并确认后才产生真正事实。
受众变更前留下的历史事实仍追加展示，不因当前适用集改变而静默隐藏。

枚举的数据库值与渠道映射继续使用稳定英文编码（例如 `TIE_BACK`），但
`FieldSpecOut` 同时下发 `option_labels` 中文词表，属性页的选择器和采信值显示中文
（例如“系带背”）。翻译的唯一来源仍是后端属性注册表；前端不维护第二张表。


## §3.87 a55 运行日志控制台整改

**诊断窗口默认不收 2xx/3xx 访问日志。** a54 挡住了 `/api/ops/` 的自指流量,
而前端有 17 处 `refetchInterval` —— 打开一个任务详情页 ≈ 5400 条/小时,
cap 5000。修一半比不修更危险:那些行标了 routine 折进计数条,`held/cap`
显示 5000/5000,看起来非常健康。`OPS_LOG_RING_ACCESS` 默认 `errors`,
`all` 保留旧行为。**归档面一个字节没动**,而界面必须把这个取舍说出来 ——
`/meta` 报 `access_mode`,页面画一行小字。窗口可以少装东西,但不许让人把
"我没收"读成"没发生"。

**一条按代码形状判的守卫,挡不住"形状对了而行为没变"。**
`test_the_stream_is_filtered_before_it_is_shaped` 断言 `_matches` 的行号小于
`_shape` 的,一直是绿的;而调用方循环里那行无条件的 `json.dumps` 还在,
对全窗 5000 条各做一遍。守卫看不见,因为它判的是代码长什么样。换成数
`json.dumps` 的调用次数,旧断言删除并在原处写明为什么删 —— 改回 AST 断言
看起来会更快更纯,而那正是它上一次失效的样子。**凡是"做了什么"能被观察的,
就别去比代码长什么样。**

**日志要答得出"哪个进程写的"。** `service` + `pid` 进顶层字段(归档面同样带,
这是本轮唯一一处动 stdout 的地方,加字段不是改字段)。默认值 `api` 意味着
**一个没报名字的 worker 会自称 api** —— 比没有这个字段更糟:它不是"不知道",
是一句错话,而且看起来完全正常。四个入口各配一条守卫。`/meta` 的
`services_seen` 从窗口里数,不写死表:写死的表会在 worker 没起来时依然列出
`worker`,而那正是要发现的事。

**排序按时间,不按到达顺序。** 多进程 fire-and-forget LPUSH,列表顺序是到达
顺序,而链路模式的横幅承诺"按时间顺读"。时间戳解析不出来的沉底并带
`ts_unparsed`,不兜底一个时间 —— 兜底会让它安静地落在某处,而"它到底该排哪"
恰恰是答不出来的。

**`shown_oldest_ts` 与 `oldest_ts` 是两个数,谁也不冒充谁。** 上一版列表被
`limit` 静默截断,而域计数按全窗算,于是左边 800 右边 200 无人解释;同页那行
"窗口里最早的一条是 X"在被截断时是在误导人。

**查看器读日志的编码必须和写入端一致。** 写入端强制 UTF-8(有一整段注释讲
中文 Windows 上 GBK 会让整条记录消失),而 CLI 用系统代码页 —— 两边对不上时
表现是安静的乱码,`json.loads` 照样成功,不报任何错。

**`core.logging.log()` 只提供,不迁移。** a54 抓到 14 处漏写 `extra_fields`,
修法是加守卫;守卫挡的是"已经写错了",这个函数挡的是"写得出错"。217 处的
机械改动会把这一轮的 diff 淹掉,所以新写的用它,老的等各自模块下次被碰到时
顺手换,守卫两边都认。

**`ai_test_runs` 不受 `LOG-CONSOLE.md` §九「不入库」约束。** 那条禁令反对的是
把高频自动写入塞进 OLTP 库,判据是触发方式与量级 —— 人点按钮触发、每天个位数、
要和提示词版本对照留 180 天,三条都不成立。已在 §九旁边加了反向指引,
不写的话下一个人会拿那一段把这张表否掉。

## §3.88 行尾策略收敛为单一实现,打包脚本对成包做行尾复验;日志控制台五修

**行尾问题不是"没有门禁",是门禁在红而包照样出去。**
`tests/pure/test_a45_batch11_fixes.py::test_no_tracked_text_file_carries_crlf`
在 a55 交付包解出的树上就是红的(2958/2959 · 1 failed),但打包这一步从不问
测试结果。实测包里 832 个文件 711 个 CRLF/混合 —— 含 `.env.example`
(331 行里 6 行混合,其中 `OPS_LOG_RING_ACCESS=errors\r` 是真的赋值)与
**白名单里的** `.github/workflows/ci.yml`。根因:`.gitattributes` 是检出期
规则,工作树比规则老、收包方从不检出、文件未被跟踪,三种情况下一个字节都
不动,而两个打包脚本忠实逐字节复制 —— 规则声明了、没生效、没人发现。

修法:**判定只有一份。** 新增 `tools/normalize_eol.py`(`--check`/`--write`,
认 CRLF/混合/**孤立 CR** —— 后者 `grep -c $'\r$'` 与 `file` 都漏);
`pack.sh` 与 `pack.ps1` 成包后**解开验真实字节**,不合规删包退非零,
找不到 python **fail-closed 拒绝出包**(跳过 = 又一个"看起来在跑"的门禁);
它进两侧必备清单(`REQUIRED`/`$Required`);全树守卫改为 importlib 委托同一份,
另加两条反向守卫钉住委托关系与折叠顺序(先 `\r\n` 再 `\r`,反过来凭空多
空行)。工作树 711 个文件已归一化。顺带修掉 `pack.ps1:312` 从未定义的
`$RepoRoot`(应为 `$Root`)—— StrictMode 下那条排障分支自己会抛。

**同批:日志控制台五修(a55 复盘)。**
① `parse_ts` 兜底路径对无时区输入返回 naive,与 aware 比较/排序抛
TypeError —— `?since=2026-08-18T09:30:00` 让 `/ops/logs` 整个 500,第三方
handler 一条无时区 ts 让整页挂掉;**"容一手第三方写法"的那条路径恰恰是
崩溃源**。现恒返 aware,无时区按本机时区解释(写入端 `formatTime` 就是
本机钟;按 UTC 会让东八区悄悄偏 8 小时,偏了没人报错,500 至少看得见)。
② 筛选级别名打错被 `level_rank` 按 INFO 兜底,`level=WARN` 静默变"不筛"
拉回整窗 —— 而那条兜底只属于**日志行自己的级别**。新增 `normalize_level`,
API 400、CLI 退 2,与 `--domain`/`--event` 的既有口径对齐。
③ `/logs` 的 `ring.held` 曾是 `len(rows)`,与 `/logs/meta` 的真实 LLEN
同名两义,且让 `read_ring` 文档里"坏行体现在 held 与实际条数的差里"成空话
(差恒为 0);统一为 LLEN,多付一次往返买那条等式成立。
④ 同秒排序 tie-break 用 seq **字符串**,`"10" < "7"`,同进程同秒内顺序
倒置,而链路横幅正保证「按时间顺读」;ts 只有秒级,`RingHandler` 文档里
"同一毫秒"的承诺本来就落不到 ts 上。新增 `seq_sort_key`(注册表里,数值序),
API 接线。
⑤ CLI 的 `exc` 被 160 字符 `_compact` 截断 —— 真正报错的最后一行恰在被
截掉的那截里;`llm_call_id` 表头与字段区打两遍。exc 全文缩进打印,
llm_call_id 只留表头。

守卫:纯测试 5 条(`test_a53_log_console.py` a56 段,行为 + 源码形状),
行为 4 条(`test_a55_log_console.py`:held=LLEN、naive 不 500、WARN=400、
同秒按计数序)。§3.82 撞号:后写的 a55 整改节改号 §3.87,
`PRD-A55` 与 `LOG-CONSOLE.md` 两处引用同步。

## §3.89 提示词注册表与内容派生版本落地(BLOCK-30 的 S1 + S3 后端核心)

**登记的是现状,不是愿望。** `app/prompts/registry.py` 把 PRD §8 那张
8 处提示词的盘点表变成机器可读:key、层(FREE/TEMPLATE/MAPPING)、默认值
引用(点分路径惰性解析,注册表自己只依赖 app.core)、消费方、可编辑性。
`editable` 的语义定为**消费链路会不会读库** —— 8 处里只有评分两份走
`get_active_content`,其余 6 处的正文由代码拼装或引用常量,对它们放行
落库得到的是「保存成功、毫无效果」,比男装那条"后端通、前端不可达"
(PRD 问题 A)更糟。保存端点按 `editable_keys()` 拦(400),`KNOWN_KEYS`
改由注册表供值且守卫钉住"与改造前逐字节相同"。男装本轮不上线,按
§14.1 走显式路径:`ui_reachable=False` + 原因 + 解锁条件(FE-301 落地)
写在 consumers 里,守卫强制这两样存在。

**版本 = 内容哈希(BE-305~307)。** `app/prompts/versioning.py` 的
`content_version = sha256[:12]`,只接受已定型的最终字符串,非 str 直接
TypeError —— 不稳定序列化造出的孤儿版本堆比「版本号不动」更糟,因为它
看起来在正常工作(AC-30b)。三条链路接线:识别删
`EXTRACTION_PROMPT_VERSION`(「改一次必须 bump 一次」不再靠人记得)、
文案删 `PROMPT_VERSION = "llm-1"`(TEMPLATE_VERSION 刻意保留 —— 那是
模板生成器的,不是 LLM 的)、出图从函数默认参数 `"v1"` 改为对拼装后
最终字符串取哈希;**无提示词的任务保持 "v1"**,存量幂等身份不漂移。
迁移 0055(`evaluations.prompt_version` Integer→String(32))文件已落,
downgrade 写明有损。

**防注入段:切片派生,不是第三份副本(BE-308)。** `ANTI_INJECTION_BLOCK`
从女装默认正文里按边界提取,再断言男装逐字包含 —— 两份默认提示词
**一个字节没动**:AC-34(女装历史样本分档逐条一致)本轮没有能跑的环境,
改写正文拼接方式的风险不背;副本漂移由 import 时的 raise 当场暴露。
`lint_prompt` 按 tier 分派(BE-304):FREE = 既有规则 + `no_anti_injection`
(内置默认自带该段,不会一打开页面就一片黄);TEMPLATE = 必需槽位指名
道姓(AC-26);MAPPING = 只验 JSON 可解析,键封闭性随 §14.2 的只读决议
一起推迟。

**API(BE-302/303)**:`GET /prompts` 返回注册表全量 + editable 项的
生效版本号;`GET /prompts/{key}/versions/{n}` 让「看一眼 v3」不再等于
「把 v3 切成线上口径」(FE-306 的后端半)。前端本轮只做 S0 两条 P0:
FE-308 清空内容不再静默(`enabled: dirty`,fail-open 关掉)、FE-311
死指路文案改为登录口径。

**本轮不做、且写明为什么**:`ai_test_runs`(迁移 0056)与 S2 全套、
S4 页面整改、FE-301~304 列表详情改版、AC-34 回归 —— 都需要
DB / node_modules / 历史样本,盲改的风险大于晚做的成本;迁移 0055 也
只落文件未在任何库上执行。守卫 14 条(`test_a56_prompt_registry.py`,
AC-23/26/30/30b/31 离线半/36),照 `test_a51` 的注册表比对模式。
顺带:四处模块级 assert 被本仓 `-O` 剥离守卫抓住,全部改 raise;
两个引用旧版本常量的既有测试改接内容派生。

## §3.90 a57:AI 测试留档落地,一份指着不存在的表的迁移,以及一条失锚了两轮的变异

**迁移 0055 在任何真实库上都跑不起来。** `_TABLE = "evaluations"` —— 全仓没有这张表。
评分域是 `rule_sets` / `candidate_evaluations` / `evaluation_problems` / `review_items`,
带 int 版本列的那张叫 `evaluation_attempts`(迁移 0009 建)。**一份从未被执行过的迁移,
它的表名没有任何东西在校验**:模型侧不知道迁移写了什么,门禁只查迁移链 head 唯一,
autogenerate 不回头看手写的 alter,而 §3.89 自己写着「迁移 0055 也只落文件未在任何库
上执行」。守卫补成全量而不是钉住 0055:任何 `op.alter_column` 的目标表,必须在本仓
的迁移里被 `create_table` 过。只收 `alter_column` 是刻意的 —— `create_index` 的表名
参数位置随写法变,收进来会带一堆假红,而假红最省事的消法是把整条门禁删掉(§3.37)。

**改一列的类型要动五个面,少一面比不改更糟。** 迁移目标表 / ORM 列类型 / 出参 schema /
前端类型声明 / **写入侧的取值函数**。最后一个最容易漏:
`prompt_version if isinstance(prompt_version, int) else None` 在列还是 Integer 时是对的,
而它同时是一个**静默丢弃器** —— 评分链路一旦接上内容哈希,落库版本会集体变空,不报错、
不告警。归一到 `prompts/versioning.version_text()`,**强制转文本而不是放宽 isinstance**:
放宽会让 int 和 str 两种形状同时存在于一列里,聚合时又要各写一次转换。顺带订正 0055
文档里的第二句理由 —— 它说「BE-305 之后版本值是 12 位内容哈希」,而 §3.89 接的三条
链路是识别 / 文案 / 出图,评分链路至今仍是自增序号。

**留档失败吞异常,但不静默。** 留档发生在模型调用之后,那次调用可能真花了钱、结果已经
在手上。抛出去 = 运营拿到 500,钱花了、结果也没了,而丢掉的本来只是一条台账。所以
`ai_test_archive.archive()` 捕获后走 `eval.ai_test_record_failed`(ERROR 级)。**这与 a53
「日志绝不反噬业务」不是同一条,方向相反**:那条说日志不许拖垮业务,这里说台账不许拖垮
已经付过费的结果。两者共用一个形状,但这里多一个义务 —— 必须留下可检索的痕迹,因为这张
表的价值恰恰在于它完整,吞掉不喊 = 一张有洞而看起来完好的档案。

**评分放指针,文案放全文。** 评分正文已在 `EvaluationAttempt`,再抄一份的代价不是磁盘是
**两份会分叉** —— 改了一处忘了另一处时没有任何东西会红,而读的人不知道信哪份。文案相反:
`diagnose_copy` 明确不建 ContentPlan / ListingCopy,今天它的输出全仓一行都没存。三条评分
出路与两条文案分支**各留档一次**,不在函数末尾合并写:前两条是 early return,合并点根本
到不了,而"失败的测试没进历史"正是这张表最该记住的那一类。

**保留期不是一个可以拍出来的数(PRD §14.3 决议)。** 一条留档的寿命下界由它引用的东西
还活不活着决定,不由日历决定。拆成可调的 `DEFAULT_RETENTION_DAYS = 180` 与不可协商的
「引用着当前生效版本就不删,多老都不删」。守卫里有一条专门验**可调的那半不许把不可协商
的那半带走**(保留期调到 0,引用还活着的记录仍然不删)。`purgeable()` 本轮刻意不接线,
也刻意不进 `WIRED_MODULES` —— 清理拍子要和 BE-311 的读接口一起才验得动。

**纯判定住 `workflows/` 而不是 `prompts/`,因为 a56 的守卫当场挡住了它。** 那条守卫要求
`app/prompts/*.py` 只许 import `app.core`,而留档成形必须 import `app.llm.redaction`
(脱敏规则只能有一份)。两个要求都对,冲突的是位置 —— 搬去 `workflows/`(本仓给零依赖
判定留的地方),**没有去放宽那条守卫**:一条为了容纳新文件而放宽的门禁,下一次就挡不住
真正该挡的东西。

**`mutate_batch14_2.py` 的 N12 失锚了两轮,什么都没验。** 它锚在 `AttributeTab.tsx` 的
`if (!item.missing_reason) continue`,而 §3.83 把那个判定整体收进后端
(`missing_summary.unresolved_missing`),那一行被删掉;守卫在同一轮跟着改了,变异没跟。
重新对准的是**同一个意图在新归属下的入口** —— 把喂给组件的数据从后端派生值换回逐图证据,
命中的是此前**没有任何变异覆盖过**的那条断言。后端那半边归 §3.83 的守卫,不在本批
`SUITE_FILTER` 射程内,已在注释里记明需要另立一份变异。

**变异验证第二次抓到窗口没封闭的守卫。** `test_both_endpoints_pass_a_real_actor...` 第一版
写的是 `"actor=current_actor(request)" in source`,而 `api/workbench.py` 另有四处端点也这么
写 —— 把文案测试那一处删掉,子串照样在,守卫照样绿,而留档从此全部署名 `system`。与
batch13-3 的 M11、a54 的 P1 同族:**窗口没封闭的守卫,验的是"这个文件里有没有人这么写过",
不是"这里写没写"**。改成按 AST 定位到那个端点函数内部。

守卫:纯测试 26 条(`test_a57_ai_test_archive.py`,成形 / 保留期不变量 / 接线 / 迁移目标表
四类,**0 skip**);变异 14/14 验红(`tools/mutate_a57.py`)。PRD 同步升 v3.0:三张表各加
状态列,§14.1 与 §14.3 就地决议,§14.4 补当前口径。

## §3.91 a58:提示词管理中心的前端半,男装那笔欠账还清,以及连着两轮由变异抓出的同一种守卫

**`ui_reachable=True` 的判据是"两份提示词走同一段代码",不是"页面上能看见它"。**
a56 把男装标成显式不可达并写下解锁条件「FE-301 落地」;a58 把详情页从写死
`vision_system_prompt` 改成按路由 `:key`,两份提示词因此走同一段代码,这才翻转。
**如果详情页里还留着任何一处写死的 key,那个 True 就是一句错话,而且比原来的
False 更糟:它看起来可达、点进去出错。** 守卫钉的正是这一条 —— 详情页代码里不许
出现 `vision_system_prompt`,连 `VISION_SYSTEM_PROMPT` 这个常量名也不许引用
(引用它意味着某处仍在假设"默认就是女装")。

**「想看一眼」和「想让它生效」被拆开了。** 在 BE-303 之前,唯一能看到 v3 正文的
办法是把它切成生效版本 —— 而那个动作会立刻改掉线上每一张图的评分口径。
`/prompts/:key/versions/:n` 是只读页,**刻意什么都不能做**:没有编辑框、没有保存、
没有"切到这版"。把切换按钮放在一个标题写着"只读"的页面上,是在训练人忽略这一页
的名字。守卫禁止它 import 三个写方法。

**不可编辑的项照样列出来,原因从注册表原样取。** 8 处里只有评分两份的消费链路读库,
其余 6 处给编辑入口得到的是「保存成功、毫无效果」。但**不给入口不等于不列出来**:
一个运营看不见的提示词,出问题时没有人会想到它的存在。原因不在前端重写一份 ——
两份说辞迟早分叉,而分叉时读的人不知道信哪份。

**顺带修掉一个只在有消费方之后才暴露的缺陷:标记不同形。** a56 给五处不可编辑的
提示词写的是 `editable=False 的原因:...`,给 `repair_prompt_additions` 写的是
`editable=False:...`(少了"的原因")。库里、注册表里都写着理由,而前端按标记取
原因取到 null,界面显示一句兜底的抱歉。**一个被五种写法表达的机器可读标记,只是
名义上可读** —— 在没有消费方的那一轮里,它长什么样都不会有人发现。

**没有「近 7 天调用次数」这一列。** PRD FE-301 要它,而后端 `list_prompts` 的文档
写着「先不给一个算不准的数」。前端不推测状态(硬规则第 4 条):后端不给就不显示,
不拿别的字段凑一个近似值。**一个凑出来的运营指标比没有指标贵得多** —— 没有指标时
人会去问,有一个错指标时人会照着它做决定。

**连着两轮,变异抓到的是同一种守卫:窗口没封闭。** a57 的 M10(actor)、a58 的
M3(列表页入口),形状完全一样 —— 断言写成 `"某个表达式" in source`,而那个表达式
在同一个文件的**别处**也出现一次,于是把该处改坏,子串照样在、守卫照样绿。
batch13-3 的 M11 是这条谱系的第一例。**判据要写成"这个位置被那个条件挡着",不是
"这个文件里出现过那个条件"** —— 前者要么按 AST 定位到函数内部,要么按字符距离
钉住相邻关系。这一类已经出现三次,值得当作写守卫时的默认自检项。

**a58 的守卫只读源码,而这一点必须写在守卫自己的文档里。** 没有 node_modules 的
环境里 `tsc` / ESLint / Vitest 一个都跑不动,而这一轮改的恰恰是前端。§3.87 立过
「凡是"做了什么"能被观察的,就别去比代码长什么样」—— 这里不违反它,因为前端行为
在这个环境里**观察不到**。等 `make fe-check` 跑得动的那天,这几条该被行为测试
**取代**而不是叠加,哪条被取代了就在原处删掉并写明为什么删(照 LG-410 的做法)。

**离线门禁在没有 node_modules 的情况下仍然抓到了三条真红。** 两条来自
`test_error_notice_ratchet.py`(新页面用 `<Alert description={readError(err)}>` 而
不是 `<ErrorNotice error={原始error}>`,原始 error 丢了,请求编号与技术详情就没有
落点),一条来自 `test_frontend_contract.py`(详情页新加的 `surfaces` 查询没有被问过
`.isError`,失败时会静默渲染成"这份提示词不存在")。**这三条正是"前端一行没跑过"
时最值得有的那种门禁** —— 它们守的是形状,而形状恰恰是这个环境里唯一能验的东西。

守卫:纯测试 14 条(`test_a58_prompt_center.py`,AC-24 / 列表 / 只读页 / 路由 /
前后端类型契约五类,0 skip);变异 12/12 验红(`tools/mutate_a58.py`)。PRD §13 与
§16 同步回填 AC-24 与 S3 的状态。

## §3.92 a59:同一种守卫缺陷第四次出现,于是把它交给机器;以及两处我自己写出来的分叉

**四次,形状完全一样。**

    batch13-3  M11   正则没锚在 JSX 开括号上,`{false && ...}` 照样匹配 —— 渲染死了、守卫还绿
    a54        P1    `"_matches" in text`,变异把调用改名成 `_matches_removed`,子串照样在
...
**前三次的处置都是记进决策日志"下次注意" —— 而 §3.90 刚把这一段写完,a58 当轮
又犯了一次。** 这不是不够小心:窗口唯不唯一取决于**被读的那个文件长什么样**,
不在写守卫的人的视野里。一个人写守卫时想的是"我要断言这件事存在",他不会顺手
去数那串字在目标文件里出现几次。这正是该由机器答的问题,而**四次之后仍然只写
文档,等于把同一课重复上第五遍**。

于是有了 `tools/audit_guard_windows.py`:对每条 `assert "<字面量>" in <某变量>`,
若那个变量装着某个文件的正文,就数那串字面量在那个文件里出现几次;大于一次即
窗口不唯一。它**上线当天抓到的第一条真洞是 a57 自己的** ——
`"eval.ai_test_record_failed" in source`,而模块文档里也提到这个码(讲的正是为什么
要喊),把 `logger.error` 整段删掉之后子串仍在、守卫照样绿。已改成按 AST 定位到
`except` 分支内部。

**用棘轮而不是逐条台账,理由要写清。** 首跑扫出 47 条(修掉 a57 那条后 46)。
逐条写"为什么它可以不唯一"意味着替 46 个我没写过、也没有逐个读过的守卫做判断,
而**写出来的那 46 条理由看起来都会很像真的**。一份编出来的豁免理由比没有豁免更贵:
下一个人会照着它推断当初的意图。抽读了 12 条,大致 11:1 分成"存在性"与"真的洞"——
存在性那一类收窄反而会挡住正确的实现(`Idempotency-Key` 在 batch.ts 里出现三次是
正常的)。所以守的是**这个数只能降**,与 `test_error_notice_ratchet.py` 同一个道理。

**射程边界写进工具自己的文档,包括一条对自己不利的。** 正文经过任何变换的
(`code = _strip_comments(_read(...))`)刻意不解析:被读的是变换后的字符串,而次数
只能在原文上数,数出来会偏多,也就是**偏向假红**。代价是 a58 自己那条
`assert "useParams" in code` 就在射程之外 —— 如实记着,而不是为了让覆盖率好看
去解析它。

**顺带修掉一个写死的数,而它在被加剧之前就已经是错的。** `verify_delivery.py` 那条
检查的名字是「CI 覆盖全部 14 条门禁」,而表里当时只有 13 条。**它在 a59 加第 14 条
之前就已经在说错话,而没有任何东西会发现** —— 这正是 `docs/STATUS.md` 第五节
「数字不写死」立的那条规矩。名字里的数字已去掉。

**两处我自己上一轮写出来的缺陷,记在这里而不是悄悄改掉。**

一、**PRD 里 S3 那格写 ✅「AC-24 达成」,而 §13 的 AC-24 那行写 🟡「浏览器未实测」。**
两句话在同一份文档、同一次改动中被写成互相矛盾 —— 而这份 PRD 加状态列本来就是为了
防 §3.70 那类分叉。订正为 🟡,并立一条:**阶段格的状态不许比它依赖的验收项更乐观。**

二、**`docs/STATUS.md` 说那 17 处 `Alert + readError`「没有任何自动化防线,只许减不许
增靠代码评审」—— 而 `test_error_notice_ratchet.py` 就是那句话要的东西**,并且它在 a58
当轮真的拦下了两个新页面。这一句错得有代价:它会让下一个人以为迁移这件事全靠自觉,
从而不去查基线该不该调。

**a57 / a58 的运营可见口径两轮都没进 STATUS.md,本轮补上。** CLAUDE.md 里那句
「阶段 6 带来过两次运营看得见的口径变更,都记在 docs/STATUS.md」是一条约定,
而我连着两轮忘了它:`/prompts` 从单页变成三页、AI 测试从"关掉页面就没了"变成留档,
两件都是运营看得见的。

## §3.93 a60:S4 的乐观锁与回滚理由,以及一次差点顺手抹掉别人欠账的改动

**「两个人同时改同一份提示词」是 a58 才变成常态的。** 在提示词只有一个入口时它几乎
不会发生;列表页把八处都摆出来之后才成为日常。**S4 依赖 S3 不只是排期,是这条风险
自己变大了。**

**三个动作都要乐观锁,不是只有保存(AC-35)。** 保存 / 切版本 / 恢复默认在这一点上
是同一类:A 在切 v3、B 同时在切 v5,一样静默覆盖。`save_version` 原有的 advisory 锁
与重试守的是**另一件事** —— 版本号别撞、active 别有两条;两个人各自基于不同版本各存
一版,对那把锁来说完全合法。检查必须**在拿锁之后**:拿锁之前读到的"当前版本"随时会变,
检查过了照样能被插进来,而那种检查比没有检查更贵 —— 有防线的地方没人会再去想并发。

**内置默认必须能被表达成"没有版本"。** 服务端此时版本是 None,调用方看到的也该是 None。
用 0 之类的哨兵值会让「回到默认」和「第 0 版」长得一样,而这两者的下一步完全不同。

**409 不能只回一句「版本冲突」。** 运营此刻要判断的是"我该放弃还是该合并",而那要知道
对方是谁、什么时候、改成了第几版。少了这三样,唯一可行的动作是刷新重来,他刚写的那段
就此没了 —— **和静默覆盖只差一个提示框**。界面上它也不走「重试」话术:`<ErrorNotice>`
只在给了 `onRetry` 时才画重试按钮,这里靠**不给**来表达,因为重试正是这里最不该做的事。

**回滚理由不写进版本行,这一条是被一次连带损伤换来的。** 第一版把理由拼到被切到那一版的
`note` 上,而 `row.note = ...` 让 `audit_column_writers.py` 按**列名**把
`ListingImageSet.note` 也算成有写入点(那个审计刻意"多认一次",宁可漏报不肯假红),
它的欠账条目当场失效并被点名 —— **一次改提示词的改动,顺手抹掉了图片集那张表上的一笔账**。
理由改走审计 payload 与 `settings.prompt_rolled_back` 事件。守卫按 AST 判赋值而不是判
源码子串:上面那段解释性 docstring 里正当地含着 `row.note = ...` 这串字,子串断言会把
自己的墓志铭当成尸体(§3.89 里那条守卫记过同一个坑)。

**事件码必须是字面量。** 第一版写成 `"settings.prompt_rolled_back" if note else
"settings.prompt_activated"`,而 `test_a53_log_console.py` 按 AST 找 `"event": "..."`,
三元表达式两边它一个都看不见 —— 于是「登记了没人写」当场变红,**而变红的是注册表那侧,
读起来像是登记多了,实际是调用点藏起来了**。改成两条字面量分支,字段表仍只有一份。
补一条守卫:两个字面量都在**不代表两条分支都走得到** —— 把条件改成 `if False:` 之后
字面量原样在、注册表也不红,所以那条守卫改判"分支条件确实读了 `note`"。

**diff 是自己写的,二十行 LCS,没有装库。** 这一页要比的是几十到几百行纯文本,没有
语法高亮与折叠需求;为它引一个依赖换来的是 lockfile 多一行、构建多一个包,以及一个
只有这一处用得到的 API。相同的行也全量显示:「第 12 行改了」这个信息只有在能看到
第 11 行和第 13 行时才有用。`diffOpen` 这个状态名从第一版起就承认这里该有对比 ——
而里面此前只有一个只读 textarea。

**a59 那条新门禁在自己的下一轮里抓到了两次。** 写 a60 守卫时,`onRetry` 那条用了
±400 字符的窗口,抓到了隔壁 `surfaces` 那个 `ErrorNotice`;`row.note` 那条用了源码
子串,抓到了自己的 docstring。两次都在合入前被自己的变异脚本挡下。**这说明门禁做对了
一半**:它管的是 `assert "x" in <文件正文>` 那一种,而窗口过宽的形态不止那一种 ——
按字符距离取窗口、按子串判 Python 代码,都在它射程之外。记在这里,不夸大它的覆盖。

守卫:纯测试 16 条(`test_a60_prompt_concurrency.py`,0 skip);变异 15/15 验红
(`tools/mutate_a60.py`)。PRD §13 / §16 回填 AC-27 / AC-29 / AC-35 与 S4 的状态,
并把 FE-310 的落地口径写在那一条下面。

## §3.94 a61:前端第一条会执行的测试通路,以及一句重复了六遍的话

**先说反思。** 从 a54 到 a60,「下一步第一件事就是 `make fe-check`」在交接文档里
重复了六遍。每一遍都写着「这是环境问题,不是排期问题」—— 而这六轮里前端改动从
1 个文件涨到 6 个文件,**全部只过源码形状守卫**。一句重复六遍的诊断不再是诊断,
是免责声明:它把"我做不到"说得像"我已经处理了"。

真正该问的是:**在这个环境里,能不能把那件事的一部分做成?** 答案是能,而且成本
接近零 —— Node 22 原生剥类型(不用 flag),`node:test` 是内置的。不装依赖、不转译、
不引框架,判定层从此有真的会跑的测试。**这个能力六轮里一直在手边,没人去试。**

**射程必须说清,而且要说三遍。** 剥类型**不做 JSX 变换**,`.tsx` 一行都跑不了。
所以这条通路覆盖的是 `.ts` 里的纯函数,组件渲染 / hooks / 交互一条都覆盖不到。
运行器的输出、Makefile 的注释、`STATUS.md` 的限制条目三处都写了这句,并且有守卫
钉着它们不许被删。理由:**一个被说大的能力比没有能力更贵** —— 它会让下一个人
不去装 node_modules,而 `fe-check` 那四层仍然欠着。

**由此长出一条新的写码约定:能不带 JSX 的部分就别带。** `diffLines` 从
`TextDiff.tsx` 搬进 `utils/textDiff.ts`,渲染留在 `.tsx`。这条分界线现在决定
一段前端代码**能不能被验证**,而不只是"好不好看"。

**调色板 token 是第二个纯静默类:`brandVars` 是 `fromEntries` 出来的 Record,
取不存在的键得到 `undefined`,而 CSS 收到 undefined 只是不上色。** 类型上 TS 会拦,
但这台机器跑不了 typecheck —— a60 当轮就写出过 `successBg` / `dangerSoft` 两个
不存在的键。离线体检加了第四格,判据取 `theme.ts` 的 `lightTokens`(唯一事实来源),
解析不出来就**红**而不是静静跳过。

**改一行 `check-offline:` 打断了三处锚点,而三处的病因是同一个:地标把会变的清单
当成了不变量。**

    tests/pure/test_a45_batch14_14  锚在 "check-offline: test-pure verify-delivery"
    tools/mutate_batch14_14  G1      锚在当时的全部前六项
    tools/mutate_a60         M13     锚在 TextDiff.tsx 里的 lcsTable(判定搬走了)

前两处编进锚点的 `test-pure verify-delivery` 是那条规则**当时的前两项** —— 每加
一条门禁就失配一次。而失配的表现是 `only_line` 抛「找错了地方」,读起来像地标写错了,
实际是地标问错了问题。改成只取规则名 `check-offline:`(注释行以 `#` 开头,
按 startswith 判天然不会抢先命中 —— 原注释说它"出现两次"是按 `in` 说的,不成立)。
收窄之后 G1 又撞上「锚点不唯一」:`.PHONY` 那一行逐字含着同样三个目标名,
于是再往右带一项 `audit-doc-refs`,它只在真规则里跟在后面。

**a61 的守卫自己又被抓了三次窗口取错**(`check-offline:` 的 split、Makefile 注释块
按固定字符数往回数、STATUS 里找脚本名而 STATUS 写的是 make 目标名)。三次都在合入前
被变异脚本挡下。连着三轮如此 —— **`audit_guard_windows.py` 管的是
`assert "x" in <文件正文>` 那一种,而"窗口取错"的形态远不止那一种**:按字符距离取窗口、
按 split 取段、按两个名字里的哪一个去找,都在它射程之外。这条如实记着,不夸大它的覆盖。

守卫:纯测试 11 条(`test_a61_frontend_pure_tests.py`,0 skip);前端行为测试 12 条
(`frontend/tests/pure/textDiff.test.ts`,**本仓前端第一批真的会执行的用例**);
变异 12/12 验红(`tools/mutate_a61.py`),另有 a57/a58/a60/batch14-14 四份复跑全绿。
`make fe-test-pure` 已按硬规则第 3 条接进 Makefile / ci.yml / 门禁清单表。

## §3.95 a62:BE-311 落地,顺带把三笔"前提已经满足"的欠账一起还了

**一笔"前提已经满足"的欠账,和一笔"还等着前提"的欠账,在台账上长得一样。**
a57 写完 `purgeable()` 刻意不接线,理由是「没有读接口,删对了没有只能直接查库看」;
a58 解锁男装时留下 FE-306 的统计列;a60 收完 S4 时它仍然没有调用点。BE-311 一落地,
这三条的前提同时消失 —— 而如果不同批还,它们会继续以"还等着前提"的样子被跳过去,
**因为台账上没有任何东西记录前提什么时候满足**。

**参数判定单独一层,理由是那三条各自对应一次真实事故的形状。**
认不出的取值静默变成"不筛"(§3.88 ②:`level=WARN` 打错被兜底成 INFO,拉回整窗,
而运营会把那一屏读成「这段时间只有这些」);时间解析失败让整个端点 500
(§3.88 ①:那条"容一手第三方写法"的路径恰恰是崩溃源);页大小没有上限。
这三件在集成测试里最难覆盖全 —— 要造一堆只差一个字母的参数组合。

**超限的页大小是收到上限,不是拒绝。** 传 `limit=100000` 的意图是"一次拿全";
拒绝它只会让调用方改成循环拉一万次。收到 100 **并在出参里如实报出生效值与上限**,
才是那个意图的正确答案 —— 不报的话调用方会以为自己拿到了全部。

**naive 时间按 UTC 解释,而日志那边按本机时区 —— 两处的正确答案不同。**
日志的写入端 `formatTime` 用的就是本机钟;这里是 API 入参,调用方可能在任何时区,
而 `created_at` 落库是 timestamptz。照抄另一处会让东八区悄悄偏 8 小时。

**清理拍子排的是干跑那一档。** 与 `cleanup_service.purge_superseded_exports` 同一条
规矩(4.1 节 H):删除不可逆,默认只报告。beat 负责让「有多少该清了」出现在运行日志里,
真删要人显式触发 —— 自动删一张带完整模型输出的档案表,风险高于它省下的磁盘。
`active_versions` 每拍现查,不缓存:缓存是那条不变量最容易被绕开的方式。

**读接口必须同批有消费方。** 一个没有页面消费的 `GET /ai-tests/runs`,和男装那条
「后端通、前端不可达」是同一个形状 —— 而这份 PRD 从 §14.1 起就在防它。所以 FE-312
(历史表)、FE-313(结果区补 `prompt_version`,此前那一行渲染四项**唯独缺它**,
而"这一版跑出什么"正是这个页面存在的理由)、FE-315(费用确认每次重置)同批交付。
页面上那句「请保持页面打开,不要刷新」一并改掉 —— **一句已经不成立的提示会让运营
继续守着页面等**,而那正是留档要解决的事。FE-315 用 `onSettled` 而不是 `onSuccess`:
失败的调用一样计费。

**否定式的源码子串断言比肯定式更容易踩自己的墓志铭 —— a62 一轮踩了三次。**
肯定式要求那串字存在,注释里多一处不影响结论;否定式要求它**不存在**,而一句解释
「为什么把它删掉」的注释必然含着它。全会话第五次之后,`audit_guard_windows.py`
把这一档也纳入射程:`assert "x" not in <源码>` 时,目标是 `.py` 就该走 AST(现成的),
是 `.ts/.tsx` 就该先剥注释,两样都没有即记一笔。基线因此涨了一批(46 → 54),
a60 自己那条当场收紧,其余按存量入棘轮。

守卫:纯测试 22 条(`test_a62_ai_test_query.py`,参数判定 / 出参 / 清理接线 / 前端消费
四类,0 skip)。**FE-314 单独留着并写明为什么** —— 评分链路的版本是
`prompt_templates.version` 自增序号,文案落的是 `模型:哈希`,两类值形状不同,
互链的口径要先定,盲接会做出一条永远匹配不上的链接。

## §3.96 a63:两列版本的语义一起塌了,而两道防线都没看见

**先说反思。** a62 一轮踩了三次"否定式断言被自己注释判红",而 a58 / a60 各踩过一次。
前四次的处置都是「修这一条 + 记一笔」,到第五次才扩门禁 —— **扩门禁本该在 a60 就做**,
那时已经是第二次,而我当时写的是「如实记着,不夸大它的覆盖」。**那句话读起来像诚实,
实际是用诚实的措辞把该做的事推掉。** 与 a61 反思里那个"重复六遍的免责声明"是同一个
模式,换了一层皮:准确的标注不等于风险被处理。

第二条:a61 建了前端行为测试通路并留下「能不带 JSX 的部分就别带」,而 a62 写历史表时
把所有列渲染逻辑内联进了 `<Table columns={...}>`,**一行都测不了**。刚修好的路,下一轮
就绕过去了。带着这条去查,查出了下面这个缺陷。

**`ai_test_runs` 的两列版本语义一起塌了。**

    prompt_version           String(32)  文档写着「内容哈希,与 §11.3 同源」
    prompt_template_version  Integer     文档写着「人可读序号」

a57 把评分的 `attempt.prompt_version`(其实是 `prompt_templates.version` 自增序号)
塞进了**哈希那一列**。后果不是一行错:`prompt_version` 从此混装两种形状(评分 "3"、
文案 "gpt-4o:a1b2c3"),而 `prompt_template_version` **从建表起就没有任何写入方、
恒为 NULL**。§3.95 结尾说「FE-314 互链的口径要先定,盲接会做出一条永远匹配不上的
链接」—— 根因就在这里,而当时我没看出来。

**两道专门为此存在的防线同时没看见它。**

`audit_column_writers.py` 的全部理由是「每一列都答得出谁写它」—— 而
`ai_test_archive` 用 `AiTestRun(**kwargs)` 构造,于是整张表被划进「判不了的模型」。
**一个 `**` 就能让一整张表退出那条审计的射程,且不会有任何征兆。**

a57 自己那条 `test_the_row_kwargs_cover_every_column_the_model_declares` 验的是
**键集**覆盖模型的列 —— 而那个键一直在、值一直是 None。**键在、值恒空**,它一直是绿的。

两侧各自的测试都绿,没有一条跨过中间那道缝。这个形状本仓记过多次
(§3.19 的注册表接缝、a57 的迁移目标表),这次是第三种变体。

**修法三件:**

    口径   评分落 `prompt_template_version`(序号),`prompt_version` 留给内容派生值。
           评分链路目前没有内容哈希,**留 None 而不是把序号塞进来** ——
           塞进来的话,BE-305 真接上那天没有任何东西能分辨哪些行是旧口径
    构造   `AiTestRun(**kwargs)` 改成逐个具名。列审计覆盖从 553 列回到 570 列,
           `AiTestRun` 退出「判不了的模型」
    守卫   新增「每一列至少有一条构造路径给出**非 None 值**」—— 两个构造器合起来判,
           而不是各自:两类留档本就不该填满同一组列,要求各自填满会逼出一堆假值

**序号转不动时返回 None,不兜底成 0。** 0 会让「没有版本」和「第 0 版」长得一样,
而 `prompt_service._assert_expected_version` 刚为同一个理由拒绝过哨兵值。

**FE-314 的口径由此定下来,代码才写。** 提示词详情页管的是评分那两份(版本是自增
序号),所以按 `prompt_key + prompt_template_version` 查 —— 那是唯一对得上的一对。
`runsQueryForVersion` 判不出参数时返回 null,页面就不画入口:**一个点进去永远是空的
链接,比没有链接更糟 —— 它会被读成「这一版没跑过测试」**。后端同一条判据,序号 < 1
直接 400。筛着的时候给「看全部」的路,否则跳过来的人会以为这一页只有这几条。

**兑现 a61 那条约定:展示判定抽进 `utils/aiTestRuns.ts`,12 条行为测试。**
其中「耗时 0 ms 不被 falsy 判成没有」这一条是真的会发生的:Mock 评分器与缓存命中
都可能是 0,而显示「—」会让人以为这次没跑。这一类缺陷源码守卫永远看不见。

守卫:纯测试 3078(a57 增 4 条、a62 增 5 条);前端行为测试 24/24(2 个文件);
变异 `mutate_a57` 14/14 复跑全红,M14 现在同时命中新旧两条守卫。

## §3.97 a64:把 a63 那个缺陷做成门禁,以及一条锚点连着两轮被同种改动打断

**先说反思。** a63 那个缺陷(`prompt_template_version` 恒为 NULL,存在六轮)是被
"带着一条反思去翻渲染逻辑"**偶然**翻到的 —— 运气,不是机制。补的守卫只管
`AiTestRecord` 一张表。**一个缺陷被修掉、而找到它的方式没有被固化,下一个同类照样
要靠运气。** 另外两条:a63 只跑了 `mutate_a57` 就交付(其余四份没跑,这次补跑全绿);
前端行为测试从 12 涨到 24 条,但覆盖的是新抽出的纯函数,页面改动本身一行没验 ——
**在扩大"可验证的一半"的同时,也在同速扩大另一半**。

**第一次尝试做错了方向,记下来。** 最先想到的是扫"值恒为字面常量的列",实测全仓
只有 8 处、且全部良性(计数器走 SQL 递增或 `+=`、`RuleSet` 是种子数据)。更要紧的是
**它抓不到 a63 那个形状** —— 那一列在构造点是 `kwargs[...]`,是动态表达式。
**一条抓不到自己 motivating case 的门禁,和一条什么都不守的门禁,在 CI 上长得一样。**
没有把它做出来。

**做出来的是另一个判据:dataclass 的字段,必须有某条路径真的给过值。**
四种算"给过":构造点的位置/具名参数、`**kwargs`(保守起见算全部)、构造后属性赋值、
`dataclasses.replace`。首跑 16 个类 —— 逐条写豁免理由等于替 16 个我没读过的数据类
做判断(§3.92 的同一个取舍),所以是棘轮。**自检把 a63 的修复退回去,审计当场变红**,
这一条本身写成守卫(`test_the_audit_catches_the_defect_that_motivated_it`)。

**同一条锚点连着两轮被同种改动打断,这次修结构。**

    第一版   锚在 check-offline 的"当时的前六项"     a61 插 fe-test-pure 时断
    a61 修法 "再往右多带一项"                        a64 插 audit-dataclass-fields 时又断

**锚在会变的清单上,修法不能是换一段同样会变的清单。** a64 把 `check-offline` 的
依赖拆成**一项一行**(续行),于是"某一项"有了一个不随清单增长而变的表示,锚点改成
一整行。代价是读那份清单的三处守卫要跟着改 —— 而那正好是把它收敛成一份判定的时机
(`_helpers.offline_gate_targets`):三处各写一遍 `startswith("check-offline:")`,
下次改结构就要再找三遍。

**a64 自己的守卫又被 `audit_guard_windows` 抓到一条**:射程说明按源码子串判,而那串字
在 docstring 与输出里各有一份 —— 改坏输出那处照样绿。**写在 docstring 里的射程没人
读得到,跑门禁的人看到的只有 stdout**,所以改成按 AST 取 `print()` 的实参判。

守卫:纯测试 3084(a64 新增 6 条);新门禁按硬规则第 3 条接进 Makefile / ci.yml /
门禁清单表;`mutate_a58/a60/a61/batch14_14` 复跑全红,锚点 643/643。

## §3.98 a65:真库那一侧也有余地 —— 迁移链能在内存里跑

**先说反思。** a64 的交接里我自己写了这句:「记录不是处理 —— a61 证明过至少有一部分
是能在环境限制内做成的,**值得再问一次真库那一侧有没有同样的余地**。」**然后那一轮
就结束了。** 已经识别出正确的问题、写下了正确的下一步,却把它留给"下一轮" ——
这和 a61 反思里批判的"重复六遍的免责声明"是同一个动作,而且更难看:那次是没想到,
这次是想到了没做。

**问了之后的答案:有余地,而且不小。** 迁移是纯声明式的 —— 56 份里 13 种 `op.*`,
只有 9 处 raw `execute` 含 DDL 且**全部是索引**。表与列的演化因此可以在内存里重放,
一行 SQL 都不用跑。它抓两类:

    步内   alter_column / add_column / drop_column 的目标表或列不存在
           create_table 重建已存在的表、drop_table 删不存在的表
    步末   最终 schema 与 ORM 的 `__tablename__` + `Mapped[...]` 逐表逐列比对

第二格是重点,而且有两次事故背书:`prompt_template.py` 顶部记着一条唯一约束
**只写在迁移里**、`create_all()` 建出来的测试库缺了它 ——「测试库和生产库的结构不一致,
比没有测试更危险:它让人以为测过了」;a57 的 BE-307 又踩了同一类(迁移改了列型而
ORM 没跟),当时的处置是"五个面一起改",**而没有任何东西在看着这五个面是否同向**。

**三条自检各造一种真实缺陷,全部抓到**:a57 手工发现的「迁移 0055 指向不存在的表」、
「ORM 加列而迁移没跟」、「迁移加列而 ORM 没跟」。这一条写成了守卫 —— 一条抓不到
自己 motivating case 的门禁,和一条什么都不守的门禁,在 CI 上长得一样。

**解析器踩了两次假红,两次都改解析而不是加豁免。**

    0020  `for table in _TABLES: for name, _ in _COLUMNS:` 两层循环 ——
          表名是循环变量,那八列重放不出来,最终报「ORM 多了 reject_* 四列」
    0049  `_COLUMNS = ("upstream_versions", ...)` 加 `for name in _COLUMNS:` ——
          第一版按**循环的形状**猜(单目标=表名、元组目标=列名),0020 对了、0049 错了

第二次的教训值得单记:**按形状猜,不如按用法读。** 正确的判据在用法里 —— 循环变量
出现在 `sa.Column(x, ...)` 第一个位置就是列名,出现在 `op.add_column(x, ...)` 第一个
位置就是表名。改完之后 41 张表逐表逐列对得上,**`解析不出来的` 为零**,而那个数
不为零时对照结论就有一块是空的(守卫单独钉着它必须为零)。

**射程写在输出里,不只写在文档里。** 只比列名集合(类型 / 可空 / 默认值看不出来 ——
BE-307 那次正是类型,值得单独做);`op.execute` 的 raw DDL 不解析(本仓 9 处全是索引,
哪天有人用它加列这里会**漏报**而不是误报)。跑门禁的人看到的只有 stdout,
守卫按 AST 取 `print()` 实参钉着这两句(a64 那条的同一个判据)。

守卫:纯测试 3092(a65 新增 8 条);新门禁按硬规则第 3 条接进 Makefile / ci.yml /
门禁清单表。`mutate_a61` / `mutate_batch14_14` 复跑全红 —— a64 把 `check-offline`
拆成一项一行之后,插入新项**没有再打断任何锚点**,那个结构性修法这一轮就兑现了。

## §3.99 a66:列型比对当天抓到一条会当场炸的缺陷,以及我自己第二次造的状态分叉

**盘点时发现的第一件事,是我自己造的。** §6 那张事件码表里
`settings.prompt_save_conflict` / `settings.prompt_rolled_back` 写着「⬜ 待 S3 前端半」,
而这两条在 **a60 就登记进 `EVENTS` 且都有调用点**。AC-36 因此也停在 🟡。

**这是同一种错的第二次** —— a62 那次是 S3 那格写 ✅ 而 §13 的 AC-24 写 🟡,a59 订正并
立了「阶段格的状态不许比它依赖的验收项更乐观」。**规矩已经立了,而立规矩的人接着又犯了
一次。** 这份 PRD 从 v3.0 起加状态列,理由是「计划表不带状态,过期时不会有任何东西
变红」;而加了状态列之后,**状态本身过期时同样没有任何东西变红**。

所以这次不只订正:§6 那张表是**机器可读**的(每行一个事件码,而 `EVENTS` 是唯一登记处),
`test_a66_prd_status_truth.py` 把两边双向钉住 —— 标 ✅ 而代码里没有会红,**代码里有了
而表里还标 ⬜ 也会红**,后者正是这次的形状。§13 与 §16 那两张表**钉不住**
(「浏览器未实测」没有代码可以佐证),这一点写进守卫自己的文档并有守卫盯着 ——
一个被说大的守卫比没有守卫更贵。

**第二件事:a65 的迁移链审计扩到列型,扩容当天抓到一条真缺陷。**

    platform_rejections.status   库里 varchar(16),ORM 声明 String(32)
    RejectionStatus.FIXED_PENDING_EXPORT   20 个字符

`platform_service.py:539` 那句写入在真实库上会被 PostgreSQL 直接拒掉 —— 运营点
「我已经改好了」,拿到一个 500。而 `models/platform_rejection.py` 那一列的注释原样写着
「原来的 String(16) 装不下,写入会被 PostgreSQL 直接拒掉」:**有人发现过这件事、改了
ORM、没有写迁移。** `create_all()` 建的测试库按 ORM 走(32,够用),真实库按迁移走
(16,不够)——`prompt_template.py` 顶部那句「测试库和生产库的结构不一致,比没有测试
更危险」的镜像:那次约束只写在迁移里,这次宽度只改在 ORM 里。迁移 0057 修。

**解析器又踩了两次假红,两次都改解析。** `sa.Numeric(precision=5, scale=2)` 的宽度在
关键字里(五列假红)、`sa.String(length=_OWNER_ID_WIDTH)` 的宽度是模块常量(一列假红)。
连同 a65 的两次,这条门禁一共踩了四次,**四次都是改解析而不是加豁免** ——
一条报着假红的门禁活不过第二轮。改完 554 列两侧都解析得出类型,不一致的只有那一处。

**顺带补了一条"不看一致性看正确性"的守卫。** 两侧一致但都太窄的话,列型比对是绿的,
而写入照样炸 —— 一致性和正确性是两件事。`test_the_rejection_status_column_really_fits_
every_enum_value` 直接拿 `RejectionStatus` 最长的值和列宽比,而那个枚举是会加成员的。

**a66 自己的守卫第三次被自己的工具挡回来**:射程说明读的是**整个文件**,于是断言自己
那两行也算命中,把 docstring 里那段删掉照样绿。改成按 AST 取模块 docstring。

守卫:纯测试 3100(a66 新增 5 条 + a65 扩 3 条);迁移 0057;`audit_migration_chain`
的射程说明与守卫同步改口(它现在比列型了,旧的那句承诺已经不成立)。

## §3.100 a67:第一次真的还了棘轮的债,而后半程不是机械替换

**四个棘轮我都用同一个理由建:「太多了,不逐条读」。** 债因此不涨,而**没有人还** ——
`ErrorNotice` 那份基线从建立起只涨过一次(a58 新页面被拦下、当轮改掉),一次没降过。
一个只拦不还的棘轮,三年后仍是同一个数,而它读起来像"在管着"。

a67 还了六处,基线 24 -> 18、窄口径 17 -> 14。

**前三处是整块替换**(Dashboard / SystemStatus / Spend)。Spend 那处顺带把手写的重试
按钮交回组件 —— 连同调用点自己记着的那条无障碍理由(「用按钮而不是无 href 的锚点:
后者不进 Tab 序列,键盘用户到不了这颗重试」)。**统一出口的意义正是这个:那条理由
归组件管,调用点不必各自记得。** 而它原来写在一个调用点的注释里,也就意味着别的
调用点没有它。

**后三处不是机械替换,它们本来就画错了**(ProductList / TaskList / AuditLog)。
错误句渲染在表格的 `emptyText` 里 —— 于是一次拉取失败在界面上和「还没有 SKU」
**长得一模一样**,而这两者的下一步完全相反:一个是重试,一个是去创建。改成表上方
一条 `ErrorNotice`,空表文案留给真的空。

**这就是 A12 真正在买的东西。** 迁移不是"把 Alert 换成 ErrorNotice"这个动作本身,
是换的过程中会被迫回答「这一格到底在说什么」——而这三处的答案是"它在说一件假话"。
如果只把那 24 处当成一份要清空的清单,这一类会被原样搬过去。

守卫:纯测试 3100(数不变,债的位置变了);棘轮的双向锁按预期把基线过期当场判红,
下调后全绿。

## §3.101 a68:一项任务从 v1.0 起就没进过任何一格,而两张表各自都自洽

**逐条对 PRD 审下来,最要紧的一条是 FE-305。**

    §12.3   防注入段抽取(BE-308 / FE-305)
    §16 S4  FE-306/307/309/310 页面整改 + BE-308 防注入 | 5 项

5 项正好是那四个 FE 加 BE-308,**FE-305 不在里面**。于是 BE-308 a56 就落了,而它的
前端那一半七轮里没有任何人认领,也没有出现在任何一条欠账里 —— 代码零实现,
`DECISIONS.md` 零提及,而我做了六轮状态回填都没发现。

**值得记的不是漏了一项,是漏的方式。** §12 按功能分组、§16 按阶段分组,**两张表各自
都自洽**:§12 里 FE-305 有条目,§16 里 S4 的计数与清单也对得上(5 项确实是 5 项)。
只有把两张表**对起来**才看得出少了谁,而这件事此前没有任何东西在做。a66 那条守卫钉的
是「状态标记不许过期」——管不到「一项任务压根没进过表」。`test_a68_prd_task_coverage.py`
钉三件:§12 的每一项都要被排进某一格、§16 不许排一项没人规定过的、**每一格的计数要与
它自己的清单对得上**(S4 那个「5」正是漏掉 FE-305 的地方,而数字是手写的,读的人多半信数字)。

**§12.5 的「成本」是 PRD 自身的矛盾,改的是说不通的那一份。** FE-312 要「成本」列,而
§11.4 的表定义里只有 `billable: bool`,**没有任何金额列** —— 金额在
`provider_usage_records` 里,要按调用关联才拿得到。就地把那一列订正成「是否计费」,
并写明真要金额是关联那张表的一项新工作。**两份文档口径不一致时,让实现去凑一个编出来
的数是最坏的选项。**

**两条已经不成立的理由,躺了六到七轮。**

    PromptVersionPage:31   「与当前生效版对比…**它需要一个 diff 组件**」——
                           `TextDiff` 是 a60 写的,理由从 a60 起不成立
    api/prompts.py:100     「调用统计…**待测试留档落地后一并做**」——
                           留档 a57 落、读接口 a62 落,前提在 a62 就消失了

第二条尤其值得记:**它和 a62 我自己发现的「三笔前提已满足的欠账」是同一个形状,而这一笔
当时没被算进去** —— 因为那次是按"我记得欠了什么"去找的,而这一笔的欠账写在一句注释里。
两条都改了措辞:前者补做,后者写明「现在只差排期,不是差什么」。

**记账错误一处:** S4 状态行把「最近一次被切回的时间与理由」记在 FE-306 名下 —— 那是
FE-310 的落地口径(a60 我自己写进 PRD 的);而 FE-306 真正欠的「与当前生效版对比」
从未出现在任何欠账里,因为那一行写的是「与默认对比已落」,而那是 FE-307。
**用相近的名字记账,比不记账更难发现。**

**落码三项**:FE-305(防注入段折叠块,后端新出 `anti_injection_block` ——
**前端不许自己从 `default_content` 里切**,切片的起止由两个头尾常量定着,
后端 import 时有 raise 护着而前端没有)、FE-306 的「与当前生效版对比」、
FE-312 补「对象」与「token」两列。

**回归时前端契约门禁抓到一条真红**:新加的 `current` 查询没被问过 `.isError`,
失败时 `current.data` 是 undefined -> 右边是空串 -> **diff 会把整份正文画成"全部被
删掉"**,而那是一句假话。这正是这一页自己 44 行起批判的 fail-open。

守卫:纯测试 3104(a68 新增 4 条);前端行为测试 26/26;五份变异脚本复跑全红。

## §3.102 a68:外部审计四条落地,以及"门禁全绿"到底证明了什么

外部审计 `CODE_AUDIT_A67.md` 针对 a67,给出 1 个 P1 + 3 个 P2,结论 NO_GO。
拿到报告时**a68 包里一条修复都没有** —— 四条原样存在,而后端纯测试 3104/3104、
七个静态审计工具全 PASS。

这件事本身就是报告的中心论点,值得原样记下来:
...
### 一、`export_at` 为什么不能用来回填(ISSUE-001)

审计报告的修复建议里,历史 `created_at` 的回填来源列了四个:

```
platform status time
export time
publish attempt time
audit time
```

**其中 export time 不能用**,而报告没有展开这一点。理由要从 `resolve_gate`
的方向推:

```
回填得太早   驳回之前的旧导出被算成"驳回之后" -> 闸门自动放行
             -> 一条还没真修的驳回被悄悄关掉。漏报
回填得太晚   什么都算不上"之后" -> 闸门继续要人工确认。误报
```

`export_at` 是**被驳回的那次导出**的时间,早于驳回本身 —— 它正好踩中漏报那一支。
而 `resolve_gate` 自己写着方向:「误报的代价是运营多勾一个框,漏报的代价是驳回
台账退化成备忘录」。

所以 0058 的三级回填每一级都保证 **≥ 真实驳回时刻**,兜底落在 `now()`。
兜底有语义损失(那些行的真实创建时刻在库里不存在第四个副本),但损失的方向是
"闸门更严",不是"闸门误放行"。

**接受一个已知的、方向正确的失真,好过一个更精确但方向可能错的估计。**

### 二、扩容一条门禁,先问它会不会报假红

`audit_migration_chain.py` 扩到比 nullable / server_default 时,两处刻意收窄:

```
可空            只认写出来的 nullable=。不写时两侧默认相反
                (sa.Column 默认可空,mapped_column 看注解),
                从注解推得出来 —— 但推出来的是门禁自己的判断,不是代码里的事实
server default  只报危险方向:ORM 有而迁移没有。反过来库照样填得上,
                而本仓有一批列走 Python 侧 default=,一起报就是一片假红
```

依据是 §3.37 第一版的教训:**假红最省事的消法是把整条门禁删掉。**

收窄之后仍然抓到了全部 4 条 —— 698 个可比对列里只有那 4 条,和外部审计独立
手工列出的完全一致。**两个独立分析对上,比任何一方单独说得响。**

### 三、门禁自己看不见的那一层

0058 第一版写成 `for column in ("created_at", "updated_at"):`,而迁移审计对
`alter_column` 只认解析得出的列名 —— 那四次 alter 对门禁**完全隐形**,
门禁继续报着已经修好的差异。

这一处的处置分两半,和 §3.37 同一条原则(改解析,不加豁免):

```
迁移侧   0058 改成逐列显式写。迁移是给人读的历史记录,
         四行重复换一条看得见的迁移,值
工具侧   alter_column 解析不出列名时记进 unresolved,不再静默跳过。
         少报好过误报是既定方向,但少报也得出声
```

**只改一头都不够。** 只改迁移,下一条循环写法的迁移会重蹈覆辙;只改工具,
0058 会以一条 unresolved 的形状留在那里,永远没被检查过。

### 四、新增的守卫不是修复的证明

`test_a68_audit_findings.py` 8 条、`test_a68_fe_pure_runtime.py` 6 条,守的都是
"那行判据还在",不是"它是对的"。这一点写进了两个文件的头部,因为它正是审计
§7.2 的论点:

```
643 anchors + 772 source guards + ErrorNotice ratchet 6/6 全绿
而 ISSUE-002 照样存在
```

ratchet 证明的是"页面换成了 ErrorNotice",不是"ErrorNotice 在 403 + onRetry
时行为正确"。真正的证明在真库测试、渲染矩阵、和 CI 里那次真实执行 ——
**而这三样在本轮执行环境里一样都跑不了。**

### 五、这一轮不能说的话

```
可以说   离线子集全绿,四条修复的静态与纯逻辑面已验证
不能说   ISSUE-001 修好了
```

`test_platform_rejection_timestamps_db.py` 那 8 条需要真库,一次都没跑过。
P0 Gate 仍未关闭,a68 **不是**正式人工测试冻结版。

守卫:纯测试 3104 + a68 审计整改新增 14 条;迁移链审计扩容后仍全绿,
且抽掉 0058 会当场变红(变异验证过)。

## §3.103 a69:报错文案是界面的一部分,而它此前不在任何门禁的射程里

### 一、为什么产物是守卫,不是那几条修改

界面走查人工报了 6 条技术化报错。守卫上线当场又抓出 9 条 —— **人工漏掉的比
找到的还多**。而且那 6 条改完之后,下一轮新写的报错仍然没有任何东西拦。

判据因此定成"可机检的形状",不是"文案写得好不好":

```
A  异常对象拼进 message      f"…{exc}"
B  让运营去调 API            POST /xxx、端点
C  回显内部 id               {*_id}、{*.id}
D  Python 语法进文案          {x!r} 的引号、sorted(...) 的列表字面量
```

四种都有明确替代写法,所以判红之后下一步是清楚的 —— **这是它敢当门禁的前提**。
文风好坏自动化判不了,也不该由门禁判,这条守卫明确不碰。

### 二、C 类的修法和另外三类不同

A/B/D 都是"换个写法":`{exc}` 换成自己写的话、`POST /spus` 换成「新建款式」、
`{fmt!r}` 换成 `「{fmt}」`。

C 不是。「SPU {spu_id} 不存在」不能换成款号 —— **记录不存在,查不出码**。
所以正确做法是这句话里压根不提它:

```
id      -> detail(只进日志)
人话     -> 说清下一步:找不到这个款,可能已被删除;请回到款式列表重新进入
```

`AppError` 的文档其实早写着这个分工(「要让前端看见的字段位置走 fields,
不要塞进 detail」),只是没人把 id 也按这条分。
...
```
AUDIENCE_LABELS          core/enums.py,女装/男装/中性
AttributeField.label     注册表,而且那一行注释写着"忘了给中文名会让
                         waistband_type 这种词悄悄出现在运营的属性表里"
spu_code                 同一个文件 812 行用的就是它
「新建款式」              界面上真实存在的那个按钮
```

**这和 a68 那四条、和这一轮的表格滚动/Tooltip 是同一个形状:约定立了,
用了一半。** 立约定的那一刻没有同时立守卫,剩下的一半就靠记性,而记性会输。

### 四、一条守卫自己踩了它要防的坑

「例外还有没有用」那条,第一版问的是"这个文件里还有没有 A~D 四类泄漏"。
答案是没有,于是它报「豁免可以删掉」—— **而豁免的理由从来不是那四类**,
是"读者本来就不是运营"。

第一版把「为什么豁免」和「豁免挡住了什么」混成了一件事。这正是本仓反复记的
那个形状:**守卫验的东西和它自称验的东西不是一回事,而它是绿的**
(§3.102 第四节说的也是这件事)。

改法是把射程缩到守得住的那一件:豁免不许指向一个已经没有对象的文件。
死掉的豁免会在有人往那个路径写新报错时悄悄放行。

### 五、这一轮不能说的话

表格横向滚动与 Tooltip **只验了源码,没验渲染** —— 本轮仍无 node_modules
与浏览器。`scroll={{ x }}` 那三个数是按列宽算出来的,不是量出来的。

守卫:纯测试 3125(a69 新增 7 条);变异验证退回任一类泄漏对应那条当场变红。

## §3.104 a70:统计的分母比统计本身难,以及"数据已齐"那句话错在哪

**本轮交付 PRD 剩下的四个功能项**:BE-302 的统计段、FE-301 的近 7 天列、
FE-306 的逐版调用列、FE-314 的反向互链。前三个是同一件事的三个出口。

### 一、a68 说「数据已齐、只差排期」,那句话的前半是错的

a68 那轮把 `api/prompts.py` 里一段过期注释订正成「数据齐了,做得了 ——
`ai_test_runs` 按 `prompt_key` + `prompt_template_version` 聚合即可」。
**订正的动作是对的,指的源是错的。**

`ai_test_runs` 只记**诊断测试**(AI 测试页手工点的那些)。拿它当调用统计,
界面上会出现一个看起来像生产流量、实际是"有人手工测了几次"的数字。
**它不会错得很显眼,只会一直偏小** —— 而运营正是拿这一列判断"要不要回滚这一版"。

PRD §12.4 写的源才是对的:`evaluation_attempts`,每一次评分请求成功与失败都留一行,
那张表的 docstring 里第三个问句就是「换了提示词之后解析失败率有没有下降」。

而那张表在本轮之前**没有 `prompt_key` 列**。只有 `prompt_version`,
而版本号是 `max(version) WHERE key = ?` **按 key 各自自增**的 ——
女装的第 3 版与男装的第 3 版在库里**一个字节都不差**。只按版本号聚合,
两份提示词的调用会加在一起,得到的数看起来完全正常,却恰好是这一列
要回答的问题的反面。

所以「数据已齐」不成立,迁移 0059 补上归属之后才真的齐。
**两份文档对同一件事口径不一致时,改的应该是说不通的那一份** ——
这条判据 a68 自己在 FE-312 那里用过(「成本」改「是否计费」),这次是它自己被用了一遍。

### 二、历史行留 NULL,不回填

能想到的两条回填路径都不成立:按受众反推(商品受众**会被改**,反推出来的是
今天的受众,不是当时评分用的那一份)、按版本号猜(两份提示词的版本号空间重叠)。

所以留 NULL,统计侧单独报 `unattributed`。与 a63 那条订正同一判据:
**序号转不动时返回 None,不兜底成 0** —— 一个编出来的归属混进统计之后,
没有任何东西能再把它分出来,而它长得和真的一模一样。

代价是统计有个**起点**,`stats_since` 如实报出去。少了这句,一个在 0059 之前
跑过 500 次的版本会顶着「调用 0 次」冒充没人用过 —— 而"真的没人用"与
"统计没覆盖到"正是这一列要分开的两件事。**前端 8 条行为测试钉的就是这一条。**

### 三、分母里混进了不是调用的东西 —— 同一个坑在同一张表上第二次

`evaluation_attempts` 里有两处写的是**整轮汇总**("这一轮一张都没评成"),
不对应任何一次 API 调用。算进去既虚增分母,又把已经逐张记过的失败再加一遍。

**计费侧早就为完全相同的一条踩过坑**:`_record_attempt` 的 `billable_provider`
只在逐张的三处传,注释写着「给它们记账会让调用数凭空翻倍」。

分它们的两个显而易见的办法都不成立,而且都是查了才知道:

    按 outcome   整轮那两处写的是 ROUND_RETRY_SCHEDULED **或** PROVIDER_ERROR,
                 而后者逐张也在写 —— 按 outcome 排会把逐张的失败一起排掉,
                 失败率凭空变好看
    按 candidate_id   那一列是 ondelete="SET NULL"。候选图被清理之后,
                      逐张行会变成 NULL,当场被误判成整轮行

所以在**写入那一刻**把事实钉进 `meta.round_level`(与 `diagnostic` 同一处):
事实写完就不再变。取数侧读的键名与写入侧由 AST 对着比 —— 拼错一个字母的后果是
全部行都被当成非整轮,而统计照样出数,没有任何东西会红。

### 四、加宽一个面之前先看这个面有多大 —— 顺手捡到一个鉴权缺口

`api/prompts.py` 开头写着「和设置页一样,**读写都要过 `require_admin`**」,
而 `list_prompts` 与 `read_prompt_version` **从落地起就漏了这个依赖**。
漏的后果不对称:`read_prompt` 要口令才给的**同一段提示词正文**,
换个地址(`/prompts/{key}/versions/{n}`)不要口令就拿得到,
而提示词决定「什么图算合格」。

发现它是因为本轮要往 `list_prompts` 加统计 —— **加宽一个面之前先看这个面有多大,
才看见它本来就不该这么大。** 两个端点各补一行 `Depends(require_admin)`。
前端本来就按管理员在用(`PromptListPage` 有 `isAuthError` 分支),所以不改前端。

### 五、`make check` 整条跑绿了一次,这是七轮以来第一次

`npm ci` 与 `pip install -e ".[dev]"` 在本轮容器里都装得上。于是从 a54 起
一直写在交接里的那句「下一步第一件事就是 `make fe-check`」终于兑现:
...
**两条真的被门禁抓出来的缺陷,都不是源码守卫看得见的**:ESLint 拦下
`<Tag color="blue">`(antd 预设调色板不受 `theme.ts` 控制,暗色模式下漂色),
`verify_delivery` 拦下六个新文件没进 Git。前者正是"连着四轮只过源码级守卫"
期间没有任何东西在看的那一类 —— 它证明那四轮的欠账是真的,不是记着好看。

**但门禁面变宽 ≠ 验收项可以翻。** 真库那一半、Playwright、`docker build`
仍然一条都没跑到,所以本轮**没有把任何一个带「浏览器未实测」「真库未跑」的
验收项翻成 ✅**,包括我自己刚落码的那几项。

### 六、仍然欠着的

    AC-25 / AC-28 / AC-24 / AC-32 / AC-33   都要浏览器,一条都没跑
    AC-29 / AC-35                           要真库并发
    AC-34(S5 全量回归)                     要历史样本 + 真库 + 视觉评分器,
                                            三样本地一样都没有
    迁移 0055 / 0056 / 0059                 **从未在任何库上执行过**

最后一条尤其要紧:0059 是本轮**唯一**碰 schema 的改动,而它没被真的跑过。
`audit_migration_chain.py` 在内存里重放了迁移链并与 ORM 逐列比对(通过),
但那验的是"声明对不对",不是"这条 DDL 在 PostgreSQL 上真的跑得过"。

守卫:纯测试 3136(a70 新增 11 条);前端行为测试 48(新增 23 条,promptStats 18 + versionPathFor 5);
`test_a58_prompt_center.py` 那条前后端契约守卫**改了模型而不是放宽它** ——
拆成行级/信封级两侧各自比对,信封字段漏进 `PromptSurfaceList` 现在也会红,
而上一版根本不看那个接口。

## §3.105 按场景优化默认提示词:先修消费链路,再修措辞(2026-08-19)

**第一条反思:「把仓库里的 prompt 都润色一遍」不是正确任务边界。** 八处提示词
里,评分系统提示词与分档阈值共同校准(§3.20 / §3.21),没有历史样本回归时改一个字
都会让分数分布平移；修复补丁表又是确定性策略,改措辞等于改自动重生动作。因此本轮
明确不碰这两类,只调整能靠离线契约验证的三条链路:出图、属性识别、Listing 文案。

**第二条反思:出图的问题先是接缝,才是措辞。** `GenerationPlan` 从建表起就有
`scene` / `pose`,后端 `compose_prompt()` 也会读,但 `GenerationPlanPanel` 的创建请求
从不提交这两个字段。后端把「场景:海边」写得再漂亮,正常 UI 动线也永远用不到。
本轮先让方案页真实提交并展示场景/姿势,再把稳定场景键展开成经过审阅的默认正文。

四个场景预设(`STUDIO_CLEAN` / `BEACH_DAYLIGHT` / `POOLSIDE_DAYLIGHT` /
`LIFESTYLE_OUTDOOR`)只写可观察约束:背景是否抢主体、服装是否被遮挡、颜色/材质
是否失真、是否出现路人/文字/水印；不堆「8K / 杰作 / 大师级」这类不可验收的词。
自定义场景仍逐字透传,不按相似词猜预设。姿势与六种角度也各有具体指令,单角度调用
额外锁定同批候选的商品颜色、图案、结构与覆盖面积。

**提示词策略变了,方案指纹必须跟着变。** 同一行方案现在会发出不同正文,若仍保留
`PLAN_FINGERPRINT_VERSION = "v1"`,旧图片集会被当作符合新策略继续复用。因此本轮
升到 v2,代价是既有生效方案管理的图片集会显示过期并要求重新生成；这是有意的失效,
不是迁移副作用。任务自身的 prompt 版本继续由最终字符串内容哈希派生。

属性识别补的是图片内文字防注入、字段唯一归属(fields / missing 二选一)、可定位证据
和色偏时不猜；Listing 文案补的是不信任 facts 等输入中的指令、selling_points 不能
越过 facts、敏感宣称边界、locale 语义、以及 `regenerate_only + previous` 的局部重写
契约。两条内容版本均由现有内容哈希自动变化,不再手工 bump。

守卫:`test_prompt_defaults_by_scene.py` 8 条；既有出图接线用例同步检查预设展开。
前端 typecheck / lint / Vitest 与后端相关纯测试均执行；评分历史样本回归未执行,
因为评分默认正文没有改动。
