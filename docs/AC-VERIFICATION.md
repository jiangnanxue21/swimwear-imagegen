...
**③ 22 条 AC 仍是 0 条完整通过；AC-18/19 已有 5-1 的部分自动证据**

| 分组 | 条数 | 卡在哪 | 解锁条件 |
|---|---|---|---|
| AC-01~09、12~17、20 | 16 | **原文不可得**。v3.0 文档不在仓库与 Git 全历史。 | 文档债:补回原文或逐条重述签认 |
| AC-10/11/18/19 | 4 | 仓内执行版已写入 PRD §14.1；AC-18/19 的 5-1 纯判定已验，生产接线与端到端验收未做。 | 按 5-2~5-5 排期完成 |
...
AC-18 / AC-19                    阶段 5:5-1 纯判定部分通过,接线/预览未验证
```
...
| AC-18 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:5-1 纯判定已验,5-2 接线未做 |
| AC-19 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:5-1 快照判定已验,5-2/5-5 未做 |
| AC-20 | ? | 否 | 未验证 |
| AC-21 | 3 | 是(§14.1) | 未验证(需真实样品 + 真库 fixture) |
| AC-22 | 2 | 是(§14.1) | 未验证(需真实抽取器调度) |

**完整通过仍是 0/22**；AC-18/19 各有一部分自动测试记录，但不足以勾选整条。
