...
AC-10 / AC-11                    阶段 5:Listing 与草稿 尚未验证
AC-10 / AC-11 / AC-18 / AC-19    阶段 5:四条均有真库自动化证据(batch19~23),真环境未验证
```
...
| AC-08 | 4 | 否 | 本轮不在范围 |
| AC-09 | 4 | 否 | 本轮不在范围 |
| AC-10 | 5 | 是(§14.1 阶段 5 仓内执行版) | 未验证(5-3/5-4) |
| AC-11 | 5 | 是(§14.1 阶段 5 仓内执行版) | 未验证(5-3) |
| AC-12 | 4 | 否 | 本轮不在范围 |
| AC-13 | 4 | 否 | 本轮不在范围 |
...
| AC-11 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:S/M/L 三行只建一个幂等单元,真库计数证明少了两次调用(batch21);并发那半由 advisory lock 覆盖,未做真并发压测 |
| AC-19 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:快照落库 + STALE 派生 + 对象/字段/动作提示 + 导出预览读同一份已存映射,四段均有真库证据(batch20/22);真环境未验 |
| AC-20 | ? | 否 | 未验证 |
