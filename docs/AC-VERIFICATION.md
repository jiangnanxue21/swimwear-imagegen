...
PRD §14.1 原先仅写「AC-01 ~ AC-20 沿用 v3.0 原文」。v3.0 文档**不在仓库与
Git 全历史里**(只到 v3.1 / v3.1.1)，所以逐字原文仍不可得。2026-08-08
阶段 5 评审后，AC-10/11/18/19 已按现行 PRD 条文写成 §14.1 的**仓内执行版**；
它们不是冒充找回的旧原文，其余 16 条仍缺原文。
...
AC-10, AC-11, AC-18, AC-19                阶段 5:Listing 与草稿(仓内执行版)
...
| 分组 | 条数 | 卡在哪 | 解锁条件 |
|---|---|---|---|
| AC-01~09、12~17、20 | 16 | **原文不可得**。v3.0 文档不在仓库与 Git 全历史。 | 文档债:补回原文或逐条重述签认 |
| AC-10/11/18/19 | 4 | 仓内执行版已写入 PRD §14.1。四条**都有自动化证据**（batch20/21/22/23，真库用例 + 变异全红），**端到端真环境验收仍未做**。 | 真环境验收 |
| AC-21 / AC-22 | 2 | **原文可得**(§14.1 逐字在),但**前提缺失**:两条都要真实样品 + 真实抽取器调度。§14.2 自己写明现有 `sample-data/` 只有 10 件女装占位图,不满足真实样品要求。 | 外部依赖债:真实样照采购/拍摄 + 抽取器就绪 |

四条阶段 5 判据现在已经可以据此写守卫；另外 16 条补文档后才能开始验，后 2 条
即使补文档也仍依赖真实样品与抽取器。三笔债的修法不同，不能再合并成一句。
...
**真环境未验证,但「完全未开工」这句话已经过期**(A45-batch30 订正)。

原文写的是「阶段 6(一体化向导)代码本仓**完全未开工**」。那是 batch25 之前的
事实;阶段 6 的五批(batch25 / 26+27 / 28 / 29 / 30)现在**全部落码**,
六条判据都有可执行的重述版(PRD §14.1)与守卫。逐条现状见 §6 的全表。

仍然为真的是**这台机器验不了**:真库 pytest、Redis / Celery 集成与浏览器
主流程本轮都没跑(见 `REVIEW-STAGE6-CONCLUSION.md` §七的四条)。
**未验证 ≠ 通过** —— 这一节的标题保持"未验证"是对的,过期的是它的理由。
...
- 「不指定模特」绕行缝 —— **已关闭**(2026-08-11 评审)。
  `generation_service.py` 的 `if "MODEL_REFERENCE" in roles:` 不再 return,
  改为 `ValidationError`;`tasks/generation_tasks._build_request` 里那条
  "拿不到模板图就退回自由上传模特图"的兜底也删了 —— 它是同一条缝的第二道门,
  而且发生在四道检查都过了之后(创建时模板是好的,执行时它可能已被停用)。
  **关的方式是这条验收项自己写的那半句:「或在无法解析时拒绝」。**
  解析路径今天不存在(`MediaAsset.consent_id` 全仓没有写入点,`MediaConsent`
  也没有受众字段),所以落点是拒绝,并把运营指向"登记成 ModelTemplate 再选"
  —— 那是唯一能执行四道检查的路径。守卫:
  `test_a45_batch11_fixes.test_the_model_reference_bypass_is_closed`
  (两道门一起钉)。**仍未在浏览器里走过**:拒绝路径的文案与动线未实测。
...
AC-10 / AC-11 / AC-18 / AC-19    阶段 5:四条均有真库自动化证据(batch19~24),真环境未验证
```

> 上面这一段原来还有一行「AC-10 / AC-11 阶段 5 尚未验证」,与紧接着的下一行
> 直接冲突 —— 同一份文件里,同一条 AC 有两个答案。A45-batch24 删掉了过期的
> 那一行。**留着两行不是"保守",是让读者自己去猜哪一行是新的。**

阶段 4 不在本文原验收范围。阶段 5 已于 batch19 开工；这里只记录新增的部分自动
证据，不把纯函数通过冒充完整 AC 通过。
...
| AC | 阶段 | 原文可得 | 本机结论 |
|---|---|---|---|
| AC-01 | 6 | **是(§14.1 阶段 6 仓内执行版,A45-batch25 签认)** | 部分:七步向导页已落码(`/wizard/:id`,batch29),每一步的面板复用详情页那七个 Tab,13 条 Vitest 组件用例。**浏览器未实测**(Playwright 在任务 24),真库未跑。建档步仍会把人送去 SPU 页 —— 那是 AC-01 允许的高级异常,理由见 §3.59 第六条 |
| AC-02 | ? | 否 | 未验证 |
| AC-03 | 2 | 否 | 未验证(需真实样品) |
| AC-04 | 2 | 否 | 未验证(需真实样品) |
| AC-05 | 6 | **是(§14.1 阶段 6 仓内执行版,A45-batch25 签认)** | 部分:判定 `wizard.check_action` + 三个写端点上的 `_ensure_action_allowed`(batch29),拒绝理由与向导上那句取自同一处;另有一条守卫钉住「没有任何请求体接受完成标记」。**闸只在 HTTP 边界**(批次走 service 层,有自己的跳过判定),真库未跑 |
| AC-06 | ? | 否 | 未验证 |
| AC-07 | ? | 否 | 未验证 |
| AC-08 | 4 | 否 | 本轮不在范围 |
| AC-09 | 4 | 否 | 本轮不在范围 |
| AC-10 | 5 | 是(§14.1 阶段 5 仓内执行版) | ~~未验证(5-3/5-4)~~ 见下方同名行 |
| AC-11 | 5 | 是(§14.1 阶段 5 仓内执行版) | ~~未验证(5-3)~~ 见下方同名行 |
| AC-12 | 4 | 否 | 本轮不在范围 |
| AC-13 | 4 | 否 | 本轮不在范围 |
| AC-14 | 6 | **是(§14.1 阶段 6 仓内执行版,A45-batch25 签认)** | 部分:七步(batch27)+ 颜色子态(batch25)+ 聚合输出(batch28)三跳都接上了,向导轨道上按步显示「哪几个颜色还差」(batch29)。**原文写的「今天零生产调用点」已过期**。真库未跑 |
| AC-15 | 6 | **是(§14.1 阶段 6 仓内执行版,A45-batch25 签认)** | 部分:七步状态、颜色子态、阻塞项与费用预估**在同一个响应里**(`/products/{id}/flow` 的 `colors` + `wizard` 块,batch28 + batch30),没有新开第二个端点 —— 「三处不得互相矛盾」由共用一个 `ctx.result` 结构保证。真库未跑 |
| AC-16 | 6 | **是(§14.1 阶段 6 仓内执行版,A45-batch25 签认)** | 部分:当前步与当前颜色住在 URL(`?step=` / `?color=`,走 `useUrlFilters`),没带时落后端算出的 `current_step`;「挂载不发任何写请求」有组件用例钉着(batch29)。**「断线重连不丢进行中任务」未验** —— 那要真实 worker |
| AC-17 | 6 | **是(§14.1 阶段 6 仓内执行版,A45-batch25 签认)** | 部分:`GET /products/{id}/impact` 在修改之前给出对象 / 效应 / 需要执行的动作,逐格取自 `stale_matrix`(batch30)。**「字段」那一维没有** —— 矩阵是 (变更源 × 对象) 的,事前给不出字段,事后那份在 `/draft/stale-reason`。「双击不建重复对象」沿用既有幂等,本批未单独验 |
| AC-18 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:合取门禁已接线,两侧各有真库用例与变异(A45-batch20);真环境未验 |
| AC-10 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:文案与草稿只消费 CONFIRMED 事实,幂等键按已确认版本集算(batch21/23);「改 SUGGESTED 不改输入」的端到端未在真环境验 |
| AC-11 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:尺码轴与**颜色轴都收口了**。S/M/L 三行只建一个幂等单元(batch21);红→蓝→红三次点击只生成两版、两个颜色的版本序列独立、复用不建内容计划(batch24 + 迁移 0051)。batch21~23 期间颜色轴恒不命中 —— 键随颜色变而存储是 SPU 一个槽,在颜色轴上来回点每次都付费。并发那半由 advisory lock 覆盖,未做真并发压测;**真实计费差异未验**(本地生成器不花钱,证据是落库行数) |
| AC-19 | 5 | 是(§14.1 阶段 5 仓内执行版) | 部分:快照落库 + STALE 派生 + 对象/字段/动作提示 + **预览与最终导出同源**,均有真库证据(batch20/22/24)。batch22 只保证了预览不重新推断,而最终导出仍按 `sort_order` 自己挑图 —— 一份合法且 READY 的图片集也会让预览显示 A 图、Excel 用 B 图;batch24 让行级图片只从已存映射翻译,真库用例含该分叉场景。运营侧渲染同批接上(4 条前端用例)。真环境未验 |

| AC-20 | ? | 否 | 未验证 |
| AC-21 | 3 | 是(§14.1) | 未验证(需真实样品 + 真库 fixture) |
| AC-22 | 2 | 是(§14.1) | 未验证(需真实抽取器调度) |

**完整通过仍是 0/22**；AC-10/11/18/19 各有一部分自动测试记录（真库),但"自动化验过"不等于"真环境验收过"，不足以勾选整条。
PRD §14.3 的准入条件「AC-01~AC-22 有自动或人工测试记录」仍未满足。

但 22 条卡在**两个互不相同的原因**上,收口时要分开算:

| 分组 | 条数 | 阻塞原因 | 性质 | 解锁条件 |
|---|---|---|---|---|
| AC-01~09、12~17、20 | 16 | 原文不可得，无法逐条验收。 | 文档债 | 补回原文或逐条重述签认 |
| AC-10/11/18/19 | 4 | 仓内执行版可得；实现与端到端证据未齐。 | 交付债 | 完成 5-2~5-5 并逐条留证 |
| AC-21 / AC-22 | 2 | 原文可得(§14.1 逐字在),但缺真实样品 + 真实抽取器调度。§14.2 写明现有 `sample-data/` 只有 10 件女装占位图,不满足真实样品要求。 | 外部依赖债 | 真实样照到位 + 抽取器就绪 |

阶段 5 四条可以立刻按仓内执行版继续；其余 16 条仍需原文或签认版；后 2 条
**必须等外部依赖**。
