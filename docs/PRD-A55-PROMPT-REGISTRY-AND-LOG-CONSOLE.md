...
| **AC-33** | 从提示词某一版能跳到该版的全部测试记录,反向亦可 | 手工验收 | 🟡 a63 落码**正向**(提示词页 → 该版跑过的测试,按 `prompt_template_version`);**反向未做**(测试记录行 → 用的是哪一版)——它要按 key 反查那一版是否还存在,而版本可能已被删;**浏览器未跑** |
...
| **S2** | 30 | BE-309~311 + FE-312~315 测试留档 + 迁移 0056 | 7 | S1、**L1** | 🟡 BE-309/310 ✅ a57;BE-311 与 FE-312/313/315 ✅ a62(清理拍子同批接线);FE-314 **正向** ✅ a63(口径见 `DECISIONS.md` §3.96:评分落序号列、文案落哈希列,两列语义此前是串的),反向未做 |
| **S3** | 30 | BE-301~304 + FE-301~304 管理中心 | 8 | S1 | 🟡 BE-301/303/304 ✅ a56,**BE-302 差「统计」那一段**(出参有 tier/consumers,没有调用统计);FE-302~304 ✅ a58,**FE-301 差「近 7 天调用次数与解析失败率」列**;**AC-24 仍是 🟡**(见 §13)—— 这一格的状态不许比它依赖的验收项更乐观。〔a68 订正:那两项此前记的是「待测试留档落地」,而留档 a57 落、读接口 a62 落,前提在 a62 就消失了 —— 现在只差排期〕 |
| **S4** | 30 | FE-305~307/309/310 页面整改 + BE-308 防注入 | 6 | S3 | 🟡 BE-308 ✅ a56;FE-305 ✅ a68(**它此前不在这一格的清单里**,见 §12.3 补记);FE-307/309/310 ✅ a60;**FE-306 差一项** —— 「查看」✅a58 / 「与当前生效版对比」✅a68 / 「内置默认钉一行」✅a60 / `useDocumentTitle` 带 label ✅a58,**只差「调用 N 次 · 失败 M 次」列**(与 FE-301 那一列同源,数据已齐、只差排期)。〔a68 订正:上一版把「最近一次被切回的时间与理由」记在 FE-306 名下,那是 FE-310 的落地口径〕 |
