...
`/workbench-review` 与 `/reviews` 不是同一个功能的两代实现,**两个都要用**:

| 你要审的东西 | 去哪 | 对象 |
| --- | --- | --- |
| 评分没自动通过的**单张候选图** | `/reviews`(审核中心顶部「候选图审核」点进去) | 一张图 |
| **整套图片集**是否可以发布 | `/workbench-review` 逐件快审 | 一个商品的一版编排 |
| **文案**是否可以发布 | 同上 | 一个商品的一版文案 |
| **属性冲突**要选哪个值 | 同上 | 一个商品的一个字段 |

判据是**审的对象不同**,不是新旧:候选图是「这张图行不行」,图片集与文案是
「这一版可以发布吗」。合并数据模型是错的,所以两边各自的页面都留着。

审核中心顶部那三条计数是唯一的汇合点:它只告诉你哪一类有几件,点进去仍然是
各自的页面。「候选图审核」那一条**通向 `/reviews`** —— 这是调用关系,不是竞争关系。


| 路径 | 为什么保留 |
| --- | --- |
| `/reviews`、`/reviews/:id` | 候选图人工审核队列与详情。**不是被取代的旧版** —— 它审的是单张候选图,入口在审核中心顶部那条计数 |
