...
> **先分清两条路。** 这一节讲的是 Header 口令(机器凭据),它决定 CLI、脚本和
> pytest 怎么认身份。**浏览器登录是另一条**,在 §4.5 —— 两者互不影响,而验收
> 权限相关的东西时该走哪条,取决于你要复现的是哪一种部署。
...
本机默认**不开**。三项全空时走上面那条 Header 口令的路,`APP_ENV=local` 下
连口令都不用填。要验收登录本身,编辑 `.env`:

```ini
ADMIN_PASSWORD=admin-local-test
OPERATOR_PASSWORD=op-local-test        # 不能和上面相同
AUTH_SESSION_SECRET=至少三十二个字符的随便什么串再补几个字够长了吧
```

```bash
# 想要一把像样的:
python3 -c "import secrets;print(secrets.token_urlsafe(48))"
docker compose up -d backend worker beat   # 三个都要重启,它们都读这三项
```

**只要填了任意一项,本机也会真的走登录** —— `ROLE_DEV` 那条免口令回落会被关掉。
这是刻意的:否则 admin/operator 的差异、退出登录、403 一个都验不到。
不想验登录时把三项清空再重启即可。
...
# {"status":"ok",...,"auth_mode":"session"}   ← 三项全空时这里是 "token"
```
...
| # | 做什么 | 该看到什么 |
|---|---|---|
| 1 | 直接开 `localhost:5173/workbench` | 被弹到 `/login`,地址栏带 `?next=%2Fworkbench` |
| 2 | 用 `operator` + 错密码登 | 一句「用户名或密码错误」;**不区分**用户名不存在与密码错 |
| 3 | 用 `operator` + 对的密码登 | 回到 **`/workbench`**(不是首页)—— `?next=` 生效 |
| 4 | 右上角身份区 → 退出登录 | 回到 `/login`;再敲 `/today` 仍被弹回登录页 |
| 5 | 用 `operator` 登入后看侧栏 | **没有「系统管理」整组**;顶栏菜单里也没有「系统设置」。运营那三组已按 a47 收敛,看不到「商品与 SKU」「生成任务」「SPU 聚合」「图片人工审核」「新建商品款式」「批量导入 SKU」这六项 |
| 5b | operator 手输 `/settings` | 打得开,页面上一句「当前账号没有管理员权限」——**不是 404**;后端同时回 403 |
| 6 | 用 `admin` 登入后开 `/settings` | 正常;页面顶部有一句「这两把口令不是你的登录密码」 |

第 5/5b 步值得多说一句:**菜单收敛与路由注册是两件事**,理由写在 `App.tsx` 的
「菜单按角色裁剪,路由不裁」一节。看不到那一组入口是对的;手输地址得到 403
而不是 404 也是对的 —— 后者会让人以为功能不存在。

## 4.6 a47 界面收口:五条要走一遍的动线

用 **operator** 登入走前五条,建任务弹窗那两条要换 **admin**。全程 Mock,不花真钱。

| # | 做什么 | 该看到什么 |
|---|---|---|
| 1 | 工作台右上角「新建」→「新建商品」 | 就地开表单;保存后**直接落到那件商品的工作台详情**、停在「素材」页签,不是回列表让你自己找 |
| 2 | 「新建」→「批量导入 SKU」,走完导入 | 成功横幅上有一颗**「回工作台」**;点它回 `/workbench`(地址上带 `?from=workbench` 才有这颗按钮) |
| 3 | 工作台详情 →「生成任务」页签 | 有**「开始出图」**按钮;表里有状态、轮次、**失败原因**三列 —— 这三样是 `/tasks` 撤出运营菜单的前提 |
| 4 | 工作台顶部切「按款」 | 一行一个款:SKU 数、颜色数、完成度、阻断、**卡点分布**;展开看 SKU 明细。**没有款级「下一步」那一列**。地址栏出现 `?view=spu`,刷新后仍在按款 |
| 5 | 撤出菜单的六条路由手输 | `/products` `/tasks` `/reviews` `/spus/new` `/workbench-import` `/workbench-spus` **全部打得开**(`/tasks` 对 operator 是 403 页,不是 404) |

### 建任务弹窗要分两次看

| # | 身份 | 该看到什么 |
|---|---|---|
| 6 | operator 点「开始出图」 | 搜不到 **seed / 提示词 / 每轮候选数 / Provider**;能看到「出图方案」只读块与「预计付费调用次数」 |
| 7 | admin 点同一颗按钮 | 多一个**默认收起**的「高级选项(工程参数 · 仅管理员)」,里面才有那四项和「绕过出图方案」开关 |

### 方案真的控制出图了吗(§5 的可复现验收)

这一条是本轮唯一一处**业务正确性**修复,值得单独走:

1. 给一个 SPU 配一份 ACTIVE 方案,角度写 `正面×2 / 背面×1`,Provider 选 `mock`;
2. 用 operator 在那个 SPU 下任一 SKU 上点「开始出图」—— 弹窗里「角度与张数」
   应当显示方案那三张,「预计付费调用次数」按 **3 张 × 轮次** 算,不是默认的 4;
3. 任务建出来之后进 `/tasks/{id}`(admin),核对 **Provider 等于方案的那一个**;
4. 在「操作审计」里找这条 `CREATE GenerationTask`,payload 里应当有
   `plan_overrides` 与 `plan_bypassed` 两个键 —— 请求与方案不一致时,
   前者会写「请求 X,方案 Y,按 Y 执行」。

**改之前这条动线的表现是**:出图按弹窗里手填的参数跑,而验收按方案检查,
于是缺背面图 → 图片集判不完整 → 运营看到「不完整」而他并没有做错任何事。

### 非管理员绕不过方案

admin 打开「绕过出图方案」提交,任务建得出来,而它的 `generation_plan_id` 与
`plan_fingerprint` **两列都是空的** —— 绕过了方案就不许再记这份方案。
operator 手改请求体传 `override_plan: true` 会拿到 **403 `AUTH_FORBIDDEN`**,
不是静默忽略:一个没有生效的开关比没有这个开关更贵。

### 一条容易误判的现象

会话过期(或后端换了 `AUTH_SESSION_SECRET` 重启)之后,页面**不会立刻**跳登录页 ——
要等下一次请求撞上 401,前端重新探一次身份,然后才跳。所以正确的复现方式是:
改完 secret 重启后端,回到页面**点一下任意操作**,而不是干等着。
