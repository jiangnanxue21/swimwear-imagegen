# 2026-08-19 a69 交接:界面走查 + 「报错就是界面文案」这条约定终于有人守了

> 决策记在 `docs/DECISIONS.md` §3.103。下方 a68 交接紧接着。

## 三件事,后两件是同一件

```
1  三张最宽的表没有横向滚动,而同仓更窄的表都给了
2  四个图标按钮没有可读名字
3  面向运营的报错里夹带技术内部表示 —— 15 条
```

## 一、表格与图标按钮:约定在,只是没铺满

`scroll={{ x }}` 按各自实际列宽算,不拍整数(口径同 `WorkbenchListPage`):
`ReviewQueuePage` 1170、`ProductListPage` 1050、`TaskListPage` 840。

图标按钮补 Tooltip 之后**无名图标按钮归零**。两处删除的措辞特意写实:

```
ImageSetTab      从这一版编排里移除（不删除素材）   红色垃圾桶默认会被读成"图没了"
SpuCreatePage    至少要保留一个颜色,这一行不能删     禁用态那句话
```

后者包了一层 `<span>` —— antd 的 Tooltip 挂在 disabled 按钮上不触发,而这里
恰恰是禁用那一态最需要解释。它补上的正是那个文件自己注释里写的
「按钮会灰掉而没有任何提示说明为什么」。

## 二、`AppError.message` 是界面文案,不是日志

`api/client.ts` 写着「后端的 message 就是写给运营的话,原样透出」。这条约定
是对的(判定只有一份),但它有个后果没人守着:**后端每写一条 message 都是在
写界面文案**,而 250 多条里没有任何门禁在看它们像不像人话。

`core/errors.py` 其实早就写了半条规矩 ——「禁止在其中拼接数据库异常原文」——
但它只是一句注释。走查抓到的实例正是它想禁的那一类:

```
受众取值不合法:'KIDS' is not a valid Audience     StrEnum 的原生英文
属性值不合字段定义 —— garment_type: …             机器字段名
不是合法取值:'花色';允许的是 ['FLORAL','SOLID']    Python 引号 + 列表字面量
SPU {spu_id} 不存在                                运营认识款号,不认识 UUID
请先用 POST /spus 建档                             界面上没有这个东西
```

**共同形状不是"文案没润色",是把内部表示直接当人话交出去。** 而每一条的
正确做法在同一个仓库里都已经存在:`AUDIENCE_LABELS`、`AttributeField.label`、
`spu_code`、款式列表页的「新建款式」按钮。不是没想到,是没铺满 —— 和这一轮
前两件事、以及 a68 那四条,是同一个形状。

**C 类的修法和另外三类不同,值得单记。** 「SPU {spu_id} 不存在」不能换成款号:
记录不存在,查不出码。所以正确做法不是"换个字段",是这句话里压根不提它 ——
id 进 `detail`(只进日志),人话里说清下一步:「找不到这个款,它可能已经被
删除;请回到款式列表重新进入」。

## 三、守卫抓到的比人工走查多一倍

我人工走查报了 6 条,守卫上线当场又抓出 **9 条**:

```
product_service.py:79        走三步建档(POST /spus)        人工漏
workbench/service.py:1783    {fmt!r}                        人工漏
workbench_batch.py:112       {raw!r}                        人工漏
copy_generator.py:810/816    {chosen!r}                     人工漏
model_template_service.py:228 {text!r} + ISO 8601           人工漏
platform_service.py:121/190  {to_status!r} / {reason_code!r} 人工漏
```

最后那条顺带把 `ISO 8601` 换成了样例(`请按 2026-08-19 这样的格式填`)——
标准号对运营也是行话。

**这就是为什么这一轮的产物是守卫而不是那 6 条修改。** 人眼扫 250 条会漏,
而且下一轮新写的报错没人拦。

## 例外表,以及它的判据第一版是错的

`_ALLOWED` 里只有两个文件(`sorting.py`、`cleanup_service.py`),理由都是
**读者本来就不是运营** —— 排序校验只有手改 URL 参数才走得到,清理是运维接口。
给它们写运营话术是把文案改得更模糊。

守「例外还有没有用」那一条,第一版判错了方向:它问的是"这个文件里还有没有
A~D 四类泄漏",答案是没有,于是它报「豁免可以删掉」。**而豁免的理由从来不是
那四类。** 第一版把「为什么豁免」和「豁免挡住了什么」混成了一件事 ——
守卫验的东西和它自称验的东西不是一回事,而它是绿的。现在只守一件守得住的:
豁免不许指向一个已经没有对象的文件。

## 自查:我自己造了一批同类问题,以及守卫第一版射程不够

**这一节是交付前的自查结果,不是事后补的反思。**

**一、我造了 7 条新的文档漂移。** 改掉「请先用 POST /spus 建档」之后,四份
DB 测试的 docstring 里还引着那句原文 —— 而"代码前进、文档留在原地"正是
ISSUE-004 那一类。已全部订正。(守卫文件里剩的两处是**来路记录**,引的是
改之前的原文,刻意保留。)

**二、守卫第一版只认 `AppError` 那六个类,漏了三条同类泄漏:**

```
AttributeValueError(spec.name, f"…{raw!r}")   reason 被接口层拼进 422 的 message
FilterRejected(f"…{text!r}")                  api/ai_tests.py 转成 400
AudienceGateResult(problems=(f"…{x!r}",))     service.py 拼进"受众校验未通过:…"
```

共同点:它们本身不是 HTTP 错误,但**下游有一个已知调用点把文本原样交给运营**。
判据因此改成"文本会不会被透出",不是"它是不是异常"。

**射程扩完当场又抓到 2 条**(`ai_test_query.py` 105/131)。

于是这条守卫在这一轮里的账是:人工走查 6 条 → 守卫第一版又抓 9 条 →
自查发现漏了 3 条 → 扩容后再抓 2 条,**共 20 条**。人工那 6 条占三成。

**三、C 类改动确认过 `detail` 不外泄。** `AppError.to_payload()` 只输出
`code` / `message` / `fields`,`detail` 只进 `main.py` 的日志 extra_fields ——
所以「id 进 detail、人话进 message」这个分工是成立的,不是把 UUID 换了个地方给运营。

## 门禁

```
纯测试            3125/3125(a69 新增 7 条)
verify_delivery   18/19(仍只差 Git 跟踪,ZIP 解包目录不是 git 树)
其余静态审计       全 PASS
前端               fe-test-pure 26/26、syntax-check 135/135
变异验证           退回任一类泄漏,对应那条当场变红
```

**a68 那笔欠账原样还在:** `test_platform_rejection_timestamps_db.py` 8 条和
`ErrorNotice` 11 条渲染矩阵仍然一次都没跑过,要真库和 node_modules。
这一轮同样没有浏览器,所以表格滚动与 Tooltip **也只验了源码,没验渲染**。

# 2026-08-19 a68 交接:a67 外部审计四条全部落地

> 审计原文 `CODE_AUDIT_A67.md`(外部,针对 a67)。决策记在 `docs/DECISIONS.md`
> §3.102。**下方 a54 交接紧接着,保留为历史记录** —— a55~a67 各轮的交接散在
> `docs/` 各 PRD 与 DECISIONS 里,这一节不假装替它们做过总结。

## 先说一件容易读错的事

**a68 这个版本号下有两批不相干的工作。** 早前那批是 `test_a68_prd_task_coverage.py`
(§12 的任务必须在 §16 某一格里出现过,来路是 FE-305 从 v1.0 起没进过任何一格)。
这一批是外部审计整改。两者没有依赖关系,凑在同一个版本号下纯属时间重合。

拿到 a67 审计报告时,**a68 包里一条修复都没有** —— 四个问题原样存在,而后端
纯测试 3104/3104、七个静态审计工具全 PASS。这件事本身就是审计的中心论点:
...
```
ISSUE-001  P1  platform_rejections 时间戳 migration 与 ORM 漂移
ISSUE-002  P2  ErrorNotice 忽略 technical.retriable
ISSUE-003  P2  fe-test-pure 依赖未声明的 Node 行为
ISSUE-004  P2  STATUS / HANDOVER 落后于 a62~a67 实现
```

### ISSUE-001:迁移 0058,以及一个"门禁自己看不见"的插曲

0019 把 `created_at/updated_at` 建成可空无默认,`TimestampMixin` 声明的却是
NOT NULL + `server_default=now()`。两个生产创建路径都不写时间戳(ORM 认为库
会填),真实库上于是拿到 NULL —— 而 `_gate_for` 把它当 `rejected_at` 交给
`resolve_gate`,那里两个循环都在 `if since is not None:` 里面。**结果是驳回
永远闭不了环**:运营重导了、重提了,闸门照样说缺。

修法:新迁移 0058,不动 0019(它可能已在某些环境执行过,改历史迁移会让同一个
revision 在两台机器上建出两种表)。

回填按精度三级降落,**每一级都保证不早于真实驳回时刻**:

```
audit_logs 里这条驳回最早的一条留痕        同事务写入,精确
LEAST(草稿 platform_status_at, resolved_at)  两者都 ≥ 真实创建时刻,取较早的
now()                                        迁移时刻,有语义损失但只会让闸门更严
```

**`export_at` 刻意没进这个列表。** 它是被驳回那次导出的时间,**早于**驳回本身;
拿它当边界,驳回之前的旧导出会被算成"之后",闸门自动放行 —— 一条还没真修的
驳回被悄悄关掉。审计报告把它列为候选,但按 `resolve_gate` 的方向推下去,
它是唯一一个不能用的。方向由那个函数自己写着:「误报的代价是运营多勾一个框,
漏报的代价是驳回台账退化成备忘录」。

`audit_migration_chain.py` 同时扩容到比 **nullable 与 server default 缺失** ——
a67 时它明说不比这两样,ISSUE-001 正好长在那个盲区。扩容后它独立算出的差异
和外部审计手工列的**完全一致**:698 个两侧都写明可空的列里只有那 4 条。

**插曲值得记。** 0058 第一版写的是:

```python
for column in ("created_at", "updated_at"):
    op.alter_column(_TABLE, column, ...)
```

而迁移审计对 `alter_column` 只认解析得出的列名,循环那一层只映射到
`add_column`/`drop_column`。于是那四次 alter 对门禁**完全隐形**,门禁继续报着
已经修好的 4 条差异 —— 这次是门禁自己看不见,不是代码有问题。

两头都改了:0058 改成逐列显式写(迁移是给人读的历史记录,四行重复换一条看得见
的迁移,值),`alter_column` 那条静默分支改成记进 `unresolved`。**少报好过误报
是这个工具的既定方向,但少报也得出声。**

### ISSUE-002:判据换了一层,不是拆掉一层

```
旧   onRetry && outcome !== 'UNKNOWN'
新   onRetry && technical.retriable
```

旧判据让 403/404/422 一律画出重试按钮,而同一张卡片的技术详情里写着
「重试不会改变结果」。**一个组件同时说两句相反的话,运营信的是按钮。**

新判据把旧判据整个包住:写超时 UNKNOWN 那一支的 `retriable` 本来就是 false
(client.ts 那里写着「不是重试无意义,是重试**不安全**」)。

旧测试没抓到,是因为它把"错误类型"和"调用点传不传 onRetry"绑在一起测:
403 那次不传,500 那次才传。**真正出问题的组合 403 + onRetry 一次都没渲染过**,
而六个 a67 迁移页面全是那个组合。新增 11 条矩阵,固定传 onRetry 只让错误类型变。

### ISSUE-003:前提从注释搬进代码

同一份代码,Node 22.16.0 下 0/2(`ERR_UNKNOWN_FILE_EXTENSION`),22.22.2 下 26/26。
默认剥类型是 **22.18.0** 才落地的。一条号称"零依赖、可重复"的门禁,在两台都装
Node 22 的机器上一红一绿。

三处一起改:runner 加 `MIN_NODE` 版本闸 + `--experimental-strip-types` 能力探测
(不硬写死,将来 flag 被移除时不至于变成 bad option)+ 把那句扩展名报错翻译成
运行时诊断;`package.json` 写 `engines.node >=22.6.0`;CI gates job 补
`actions/setup-node` 并**钉到小版本 22.18.0** —— 钉 `"22"` 只保证"是 22",
而这条门禁分不清的恰好是 22 的内部。

### ISSUE-004:这一节本身

`STATUS.md` 两处订正(AI Test 读接口/历史页已落码、ProductList/ReviewQueue 的
URL filter 已搬),以及顶上这一节。

修文档时又撞到同一个形状:`audit_migration_chain.py` 的自述写着「不比可空/默认值」,
而工具已经在比了 —— 是这一轮新写的守卫抓到的。**代码前进、自述留在原地**,
就是审计说它会误导下一轮判断的那件事。

## 三件必须知道的事

**一、这一轮新增的守卫,没有一条是它对应问题的证明。**

`test_a68_audit_findings.py` 守的是"那行判据还在""那条迁移还在""那个比对还在",
不是"它是对的"。真正的证明分别在真库测试、渲染矩阵、和 CI 里那次真实执行。
审计报告 §7.2 花了整节说这件事:643 anchors + 772 guards + ratchet 6/6 全绿,
而 ISSUE-002 照样存在。把守卫读成证明,正好重演这一轮要修的错误。

**二、P0 Gate 仍然没关,而且这一轮没有能力关它。**

本轮执行环境没有 PostgreSQL、Redis、node_modules、浏览器。所以:
...
`test_platform_rejection_timestamps_db.py` 那 8 条**一次都没跑过** ——
它们需要真库。这是这一轮最大的一笔欠账,而且它就是审计的核心论点:
「把现有大量源码正确性证明再往真实 PostgreSQL、Redis、浏览器行为推进一步」。

下一步第一件事:在有真库的机器上跑

```
alembic upgrade head
pytest tests/test_platform_rejection_timestamps_db.py
```

那 8 条里有 3 条是行为闭环(建驳回 → 改并重导 → 闸门满足 → RESOLVED),
另有 1 条把时间戳手工抹成 NULL 断言闸门当场卡住 —— **后者是反证**,
它红了说明前面那些绿得没有意义。

**三、`ErrorNotice` 的 11 条矩阵与 0058 的回填 SQL,都还没有被执行过。**

前者要 Vitest(无 node_modules),后者要真库。两者的共同点:**它们是这一轮
唯一能证明修复有效的东西,而它们恰好都跑不了。** 不要因为纯测试和静态审计
全绿就把 a68 当成"已验证"。
