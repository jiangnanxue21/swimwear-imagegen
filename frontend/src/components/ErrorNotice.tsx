 ...
 * **按非管理员显示**。
 *
 * **这里原来写的是"答不上来退回『本浏览器填了管理口令没』,可以被本地伪造"**
 * —— 那条降级随口令链一起删掉了(理由写在 `useIdentity` 头部),
 * 今天没有任何本地状态能把浏览器说成管理员。
 *
 * 而结论不变,只是理由更短了:即便这一层判错,多看见的也只是一个错误码和
 * 一句排查建议,里面**没有任何秘密**(口令、密钥、异常原文都不在错误体里)。
 * 所以它不需要是权限边界,也不该被做成权限边界。
 */
