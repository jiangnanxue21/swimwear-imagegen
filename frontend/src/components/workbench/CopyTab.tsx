...
export default function CopyTab({
   ...
   *
   * **加新依赖的时候要回到这里。** disable 盖的是整个依赖数组:将来这个
   * effect 要跟着第三个东西走(比如换了草稿版本也该重置),规则本该提醒的
   * 那一次会被它一起吞掉,而表现是"某些情况下不重置",复现不了。
   * 触发条件只该有 `id` 与 `version` 两项,新增之前先确认它是**触发条件**
   * 而不是**结果** —— 后者进依赖就会把运营正在敲的字覆盖掉。
   */
  useEffect(() => {
    setTitle(copy?.title ?? '')
