 ...
 *
 * **没有任何本地状态能把一个浏览器说成管理员。** 口令链随浏览器登录一起退役
 * (见 `api/client.ts` 的退役说明与下面 `enabled` 的注释),所以"答不了"这一档
 * 只有一个来源:后端还没答。
 *
 * **这是显示层的判断,不是权限边界**。
...
export function useIdentity(): Identity {
   ...
   *
   * `enabled` **只依赖 `!health.isError`**,不许再挂「本地存了口令没有」那一类条件:
   * 登录成功、Cookie 也发了,而本地口令是空 —— whoami 一次都不发,顶栏永远停在
   * "未登录",菜单永远按非管理员渲染,而**没有任何请求失败**,横幅也不会说话。
   * §3.43 那一族:上游全对,断在最后一跳。
